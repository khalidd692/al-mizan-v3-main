...
(function () {
  ...
  function renderHadithCore(data) {
    ...
    // BUGFIX: Gestion complète des grades avec logging du grade brut
    const gradeRaw = data.grade_raw || "unknown";
    const grade = gradeRaw.toLowerCase();
    verdictBanner.className = "mz-verdict-banner";

    // Mapping complet des grades arabe/français vers classes CSS
    if (grade.includes("صحيح") || grade.includes("sahih")) {
      verdictBanner.classList.add("sahih");
    } else if (grade.includes("حسن") || grade.includes("hasan")) {
      verdictBanner.classList.add("hasan");
    } else if (grade.includes("ضعيف") || grade.includes("daif") || grade.includes("weak")) {
      verdictBanner.classList.add("daif");
    } else if (grade.includes("موضوع") || grade.includes("mawdu") || grade.includes("fabricated")) {
      verdictBanner.classList.add("mawdu");
    } else if (grade.includes("منكر") || grade.includes("munkar")) {
      verdictBanner.classList.add("munkar");
    } else if (grade.includes("شاذ") || grade.includes("shadh")) {
      verdictBanner.classList.add("shadh");
    } else {
      verdictBanner.classList.add("unknown");
    }

    // Logger le grade brut reçu pour debug
    console.log(
      "[GRADE_DEBUG] grade_raw reçu:",
      gradeRaw,
      "| Classe CSS:",
      verdictBanner.className,
    );
    verdictBanner.textContent = gradeRaw !== "unknown" ? gradeRaw : "Grade non disponible";
  }
