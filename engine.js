...
console.log(
  "%c ✅ Mîzan v21.1 : Prêt pour Production — Triple Bouclier + Whitelist Sahih",
  "color: #00ff00; font-weight: bold;",
);
console.log(
  "%c 🛡️ TRIPLE BOUCLIER ACTIF v21.1 — Whitelist Sahih + Anti-Doublon + Fallback Souverain",
...
var _RE_MAWDU =
  /موضوع|باطل|مكذوب|لا أصل له|ليس له أصل|ليس لهذا|كذب|منكر|شاذ|متروك|تالف|ضعيف جد[اً]|لا يصح|لا يثبت|ليس بحديث|لا يصح حديثا|ليس بحديث مرفوع|كلام|من قول|ليس من حديث/;
...
function _mzFallbackLigneeOr(containerId) {
  ...
  if (
    zone.querySelector(".mzAr") ||
    zone.querySelector(".mzC4-rw") ||
    zone.querySelector(".mzAr-orb")
  )
    return;
  ...
  zone.innerHTML =
    '<div style="min-height:40px;padding:14px 20px 12px;background:rgba(10,6,0,.92);">' +
...
function _renderTopicList(hadiths, query) {
  ...
  var html =
    '<div class="mz-source"><span class="mz-source-dot" style="background:#c9a84c;box-shadow:0 0 6px rgba(201,168,76,.5);"></span><span class="mz-source-text">' +
    hadiths.length +
    " RESULTAT(S) · DORAR.NET · AL MIZAN</span></div>";
  ...
  hadiths.forEach(function (h, idx) {
    ...
    var tg = _isTawaqquf
      ? {
          key: "TAWAQQUF",
          color: MZ_COLORS.TAWAQQUF,
          colorBg: "rgba(167,139,250,.06)",
          colorBd: "rgba(167,139,250,.22)",
          cssClass: "v-TAWAQQUF",
        }
    ...
    if (h.pertinence && h.pertinence.length > 3) {
      var pertCol = /^OUI/i.test(h.pertinence)
        ? "#2ecc71"
        : /^PARTIEL/i.test(h.pertinence)
          ? "#f39c12"
          : "#e74c3c";
      pertHtml =
        '<div style="margin-bottom:10px;"><span class="mz-pertinence" style="background:' +
        pertCol +
        "18;color:" +
        pertCol +
        ";border:1px solid " +
    ...
    html +=
      '<div class="mz-card" id="topic-card-' +
      idx +
      '" style="animation:argCardIn .4s ease both;animation-delay:' +
      idx * 0.1 +
      's;">';
    ...
    /* Matn arabe */
    html += '<div class="mz-matn">' + '<div class="mz-matn-ar">' + h.ar + "</div>";
    ...
    if (h.grade_explique) {
      html +=
        '<div class="mz-illah">' +
        '<span class="mz-illah-label">' +
        hukmLabelD +
        "</span>" +
        '<div class="mz-illah-text">' +
        h.grade_explique +
        "</div>" +
        "</div>";
    }
    ...
    if (!isnadPipeHtml && h.jarh_tadil) {
      ...
      if (cascadeVis) {
        ...
        niv3 =
          ...
          '">' +
          _mzAcc(
            "JARH WA TA'DIL \u2014 Evaluation des transmetteurs",
            "\u0633\u0650\u0644\u0633\u0650\u0644\u064E\u0629\u064F \u0627\u0644\u0625\u0650\u0633\u0646\u064E\u0627\u062F",
            "#5dade2",
            "<p>" + h.jarh_tadil + "</p>",
            true,
    ...
    if (hasN3) {
      html += '<div class="mz-niv3">' + niv3 + "</div>";
    } else {
...
function _mapHadithRaw(h) {
  ...
  if (
    h.grade_by_mohadd &&
    typeof h.grade_by_mohadd === "object" &&
    !Array.isArray(h.grade_by_mohadd)
  ) {
    var parts = [];
  ...
  if (typeof h._authority_score === "number") {
    ...
    if (/صحيح البخاري|الجامع الصحيح|بخاري|صحيح مسلم|مسلم|bukhari|muslim/i.test(_authBlob))
      authorityScore = 100;
    else if (/موطأ مالك|موطأ|مالك بن أنس|مالك|muwatta|malik/i.test(_authBlob)) authorityScore = 90;
    else if (
      /مسند أحمد|ابن حنبل|أبو داود|ترمذي|نسائي|ابن ماجه|أحمد|dawud|tirmidhi|nasa|ibn maja|ahmad/i.test(
        _authBlob,
      )
    )
      authorityScore = 80;
...
function _imperializeGradeExplique(raw) {
  ...
  segments.forEach(function (seg, i) {
    ...
    if (seg.key === "statut") {
      ...
      html +=
        "<span class='" + boxCls + "'>" + seg.content.replace(/<[^>]+>/g, "").trim() + "</span>";
    } else {
...
function _mzMd(txt) {
  ...
  return (
    String(txt)
      ...
      .replace(
        /^#{1,3}\s+(.+)$/gm,
        '<strong style="font-family:Cinzel,serif;letter-spacing:.1em;font-size:10px;color:rgba(212,175,55,.8);">$1</strong>',
      )
      /* Séparateurs --- */
      .replace(
        /^---+$/gm,
        '<hr style="border:none;border-top:1px solid rgba(201,168,76,.15);margin:8px 0;">',
      )
      /* **bold** */
...
function _enrichCardSSE(idx, h) {
  ...
  if (!card) {
    console.warn("[SSE] card topic-card-" + idx + " not found");
    return;
  }
  /* BOUCLIER UI — ANTI-DOUBLON : empêche le double enrichissement SSE */
  if (card.dataset.enriched === "1") {
    console.warn("[SSE] card " + idx + " déjà enrichie — ignoré");
    return;
  }
  ...
  if (h.french) {
    ...
    if (frZone) {
      ...
      if (arEl) {
        ...
        d.className = "mz-matn-fr";
        d.innerHTML = _mzMd(h.french);
        d.style.animation = "vIn .4s ease";
        arEl.parentNode.insertBefore(d, arEl.nextSibling);
      }
    }
  }
  ...
  if (h.grade_explique) {
    ...
    if (hukmZone && hukmText) {
      ...
      if (illahEl) {
        illahEl.innerHTML = rendered;
        illahEl.style.animation = "vIn .4s ease";
      }
    }
  }
  ...
  if (h.authority_score === 100 && !card.querySelector(".mz-sahihayn-badge")) {
    ...
    sahihayn.style.cssText =
      "margin:6px 18px 10px;padding:11px 16px;" +
  ...
  if (h.pertinence && /^(OUI|PARTIEL|NON)/i.test(h.pertinence)) {
    ...
    if (pertEl) {
      var pc = /^OUI/i.test(h.pertinence)
        ? "#2ecc71"
        : /^PARTIEL/i.test(h.pertinence)
          ? "#f39c12"
          : "#e74c3c";
      pertEl.textContent = h.pertinence.substring(0, 60);
      pertEl.style.background = pc + "18";
      pertEl.style.color = pc;
  ...
  requestAnimationFrame(function () {
    ...
    if (
      !isnadZone.querySelector(".mzAr") &&
      !isnadZone.querySelector(".mzC4-rw") &&
      !isnadZone.querySelector(".mzAr-orb")
    ) {
    ...
    var arbrePromise = new Promise(function (resolve) {
      setTimeout(function () {
        ...
        try {
          ...
          console.warn("[M\u00eezan] arbrePromise:", errArb.message);
        }
        resolve(result);
      }, 0);
    });
    ...
    setTimeout(function () {
        ...
        if (h.sanad) {
          ...
          if (nbAbsent > 0) {
            ...
            sLbl =
              nbEtablie +
              "/5 \u00c9TABLIES" +
              (nbPartiel ? " (" + nbPartiel + " PARTIELLE(S))" : "");
          } else {
          ...
          z2Html +=
            '<details class="mz-details" style="margin:8px 18px;">' +
            '<summary class="mz-details-sum" style="color:' +
            sCol +
            ";cursor:pointer;font-family:Cinzel,serif;" +
            "font-size:9px;letter-spacing:.18em;list-style:none;padding:10px 14px;border:1px solid " +
            sCol +
            "33;" +
            ...
            sCol +
            "22;border-top:none;border-radius:0 0 8px 8px;" +
      ...
      if (secAcc) {
        ...
        function _mzAmanaAcc(title, content, color, openByDefault) {
          ...
          if (c === "#5dade2")
            rgba = function (a) {
              return "rgba(93,173,226," + a + ")";
            };
          else if (c === "#9b59b6")
            rgba = function (a) {
              return "rgba(155,89,182," + a + ")";
            };
          else if (c === "#2ecc71")
            rgba = function (a) {
          ...
          return (
            '<details class="mz-details"' +
            (openByDefault ? " open" : "") +
            ' style="margin:8px 18px 4px;">' +
            '<summary class="mz-details-sum" style="color:' +
            c +
            ";cursor:pointer;font-family:Cinzel,serif;" +
            "font-size:9px;letter-spacing:.18em;list-style:none;padding:10px 14px;border:1px solid " +
        ...
        z3Html += _mzAmanaAcc(
          "\ud83d\udcd6 VOCABULAIRE \u2014 GHAR\u012aB AL-HAD\u012aTH",
          h.gharib,
          "#5dade2",
          false,
        );
        z3Html += _mzAmanaAcc(
          "\ud83c\udfdb\ufe0f CONTEXTE \u2014 SABAB AL-WUR\u016bD",
          h.sabab_wurud,
          "#9b59b6",
          false,
        );
...
function _advanceStep(id, forceIdx) {
  var idx = forceIdx !== undefined ? forceIdx : _STEP_MAP[id] !== undefined ? _STEP_MAP[id] : -1;
  if (idx < 0) return;
  _currentStepIdx = idx;
...
function _mzZoneAccAppend(containerId, title, content, color) {
  ...
  cont.insertAdjacentHTML(
    ...
    '<details class="mz-details" style="margin:8px 18px;">' +
      ...
      "list-style:none;padding:10px 14px;border:1px solid " +
      c +
      "33;" +
      "border-radius:8px;background:" +
      c +
...
function _renderDorarCards(rawHadiths, query) {
  ...
  var html =
    '<div class="mz-source"><span class="mz-source-dot" style="background:#c9a84c;box-shadow:0 0 6px rgba(201,168,76,.5);"></span>' +
    '<span class="mz-source-text">' +
    rawHadiths.length +
    " R\u00c9SULTAT(S) \u00b7 DORAR.NET \u00b7 AL MIZ\u00c2N</span></div>";
  ...
  rawHadiths.forEach(function (r, idx) {
    ...
    html +=
      '<div id="hukm-zone-' +
      idx +
      '" style="display:none;padding:0 18px 14px;">' +
      '<div class="mz-illah-text" id="hukm-text-' +
      idx +
      '"></div>' +
      "</div>";
    ...
    if (r.grade_albani)
      html +=
        '<div class="mz-grade-detail"><span class="mz-grade-label">al-Albānī :</span> ' +
        r.grade_albani +
        "</div>";
    if (r.grade_by_mohdith)
      html +=
        '<div class="mz-grade-detail"><span class="mz-grade-label">Muḥaddith :</span> ' +
        r.grade_by_mohdith +
        "</div>";
    if (r.grade_explanation) html += '<div class="mz-grade-expl">' + r.grade_explanation + "</div>";
    if (r.book_name_fr || r.book_name_ar) {
      var bookLabel = r.book_name_fr || r.book_name_ar;
      var numLabel = r.hadith_number ? " · n° " + r.hadith_number : "";
      html += '<div class="mz-source-detail">' + bookLabel + numLabel + "</div>";
    }
    ...
    if (r.source_url)
      html +=
        '<a class="mz-source-link" href="' +
        r.source_url +
        '" target="_blank" rel="noopener">Source originale ↗</a>';
        ...
        while (true) {
          ...
          for (var li = 0; li < lines.length; li++) {
            ...
            if (evtName === "error") {
              ...
              if (box) {
                box.innerHTML =
                  '<p style="color:#ef4444;padding:1.5rem;text-align:center;font-size:0.95rem">&#9888;&#65039; ' +
                  (msg.message || "Erreur serveur") +
                  "</p>";
                box.classList.add("active");
              }
              evtName = "";
                  ...
                  if (msg.chain && msg.chain.length > 0) {
                    var pipeLines = msg.chain.map(function (n, k) {
                      return (
                        ...
                        (n.verdict || "") +
                        "|" +
                        (n.death_h ? String(n.death_h) : "")
                      );
                    });
                    var treeHtml = _mzIsnadFromPipe(pipeLines.join("\n"), "SAHIH");
                    if (treeHtml) {
                      iz2.innerHTML = treeHtml;
                      iz2.style.animation = "vIn .5s ease";
                    }
                  } else {
                    iz2.innerHTML =
                      "<div style=\"padding:14px 20px;font-family:'Cormorant Garamond',serif;font-size:13px;color:rgba(212,175,55,.45);font-style:italic;\">Analyse en cours…</div>";
                  }
                }
              }
              evtName = "";
            ...
            if (evtName === "zone_3") {
              if (msg.type === "hadith_core" && msg.data) {
                ...
                if (box) {
                  box.innerHTML =
                    '<p style="color:rgba(201,168,76,.6);padding:2rem;text-align:center;' +
            ...
            if (evtName === "zone_4") {
              var src4 =
                msg.schema && msg.schema.source_principale
                  ? "Source principale : " + msg.schema.source_principale
                  : null;
              _mzZoneAccAppend(
                "sanad-acc-0",
                "AL-TAKHRĪJ — Attribution des sources",
                src4,
                "#9b59b6",
              );
              _advanceStep("TAKHRIJ");
            ...
            if (evtName === "zone_5") {
              _mzZoneAccAppend(
                "sanad-acc-0",
                "SHURUT AS-SIHHAH — 5 Conditions d'authenticité",
                null,
                "#5dade2",
              );
              evtName = "";
            ...
            if (evtName === "zone_6") {
              ...
              _mzZoneAccAppend(
                "sanad-acc-0",
                "AL-ʿILAL — Défauts cachés de la chaîne",
                ilalTxt,
                "#ef4444",
              );
              evtName = "";
            ...
            if (evtName === "zone_12") {
              _mzZoneAccAppend(
                "sec-acc-0",
                "★ ĀTHĀR AS-SAHĀBAH — Propos des Compagnons",
                null,
                "#d4af37",
              );
              evtName = "";
            ...
            if (evtName === "zone_13") {
              _mzZoneAccAppend(
                "sec-acc-0",
                "★ ĀTHĀR AT-TĀBIʿIĪN — Propos des Successeurs",
                null,
                "#c9a84c",
              );
              evtName = "";
            ...
            if (evtName === "zone_14") {
              _mzZoneAccAppend(
                "sec-acc-0",
                "🏛️ POSITIONS DES IMĀMS — Fiqh al-Hadīth",
                null,
                "#f39c12",
              );
              evtName = "";
            ...
            if (!evtName) {
              if (msg && msg.step) {
                _advanceStep((msg.step || "").toUpperCase());
                continue;
              }
              if (
                !dorarOK &&
...
function omniSearch(val) {
  ...
  window._mzOmniDebounce = setTimeout(function () {
    _omniSearchRun(val);
  }, 400);
}
function _omniSearchRun(val) {
  ...
  try {
    PREACHERS.forEach(function (p) {
      ...
      if (
        ...
        ph.indexOf(q) !== -1 ||
        vd.indexOf(q) !== -1 ||
        (p.nomAr && p.nomAr.indexOf(val) !== -1)
      ) {
        var clr = p.color || "#c9a84c";
        html +=
          "<div onclick=\"document.getElementById('omni-results').style.display='none';openDetail(" +
          p.id +
          ')" style="display:flex;align-items:center;gap:10px;padding:10px 14px;border-bottom:1px solid rgba(201,168,76,.06);cursor:pointer;transition:background .2s;" onmouseenter="this.style.background=\'rgba(201,168,76,.06)\'" onmouseleave="this.style.background=\'transparent\'">';
        html +=
          '<span style="font-family:Cinzel,serif;font-size:5.5px;font-weight:700;padding:3px 8px;border-radius:2px;background:rgba(201,168,76,.08);border:1px solid rgba(201,168,76,.2);color:#c9a84c;flex-shrink:0;">PROFIL</span>';
        html +=
          '<div style="flex:1;min-width:0;"><p style="font-family:Cinzel,serif;font-size:9px;font-weight:700;color:#c9a84c;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">' +
          p.nomFr +
          "</p>";
        html +=
          '<p style="font-family:Cormorant Garamond,serif;font-size:10px;color:' +
          clr +
          ';opacity:.7;">' +
          p.verdict +
          "</p></div></div>";
  ...
  try {
    FIRAQ.forEach(function (f) {
      ...
      if (
        ...
        ds.indexOf(q) !== -1 ||
        ph.indexOf(q) !== -1 ||
        (f.ar && f.ar.indexOf(val) !== -1)
      ) {
        html +=
          "<div onclick=\"document.getElementById('omni-results').style.display='none';openFiraqDetail(" +
          f.id +
          ')" style="display:flex;align-items:center;gap:10px;padding:10px 14px;border-bottom:1px solid rgba(201,168,76,.06);cursor:pointer;transition:background .2s;" onmouseenter="this.style.background=\'rgba(153,27,27,.08)\'" onmouseleave="this.style.background=\'transparent\'">';
        html +=
          '<span style="font-family:Cinzel,serif;font-size:5.5px;font-weight:700;padding:3px 8px;border-radius:2px;background:rgba(153,27,27,.1);border:1px solid rgba(153,27,27,.25);color:#991b1b;flex-shrink:0;">SECTE</span>';
        html +=
          '<div style="flex:1;min-width:0;"><p style="font-family:Cinzel,serif;font-size:9px;font-weight:700;color:' +
          f.color +
          ';white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">' +
          f.nom +
          "</p>";
        html +=
          '<p style="font-family:Cormorant Garamond,serif;font-size:10px;color:rgba(220,200,160,.5);">' +
          f.danger +
          "</p></div></div>";
        count++;
      }
  ...
  try {
    MYTHES.forEach(function (m) {
      ...
      if (hd.indexOf(q) !== -1 || ex.indexOf(q) !== -1 || gr.indexOf(q) !== -1) {
        ...
        html +=
          "<div onclick=\"document.getElementById('omni-results').style.display='none';openMythDetail(" +
          m.id +
          ')" style="display:flex;align-items:center;gap:10px;padding:10px 14px;border-bottom:1px solid rgba(201,168,76,.06);cursor:pointer;transition:background .2s;" onmouseenter="this.style.background=\'rgba(239,68,68,.06)\'" onmouseleave="this.style.background=\'transparent\'">';
        html +=
          '<span style="font-family:Cinzel,serif;font-size:5.5px;font-weight:700;padding:3px 8px;border-radius:2px;background:' +
          catClr +
          "15;border:1px solid " +
          catClr +
          "30;color:" +
          ...
          "</span>";
        html +=
          '<div style="flex:1;min-width:0;"><p style="font-family:Cinzel,serif;font-size:8.5px;font-weight:700;color:#c9a84c;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">' +
          m.hadith.substring(0, 50) +
          "</p>";
        html +=
          '<p style="font-family:Cormorant Garamond,serif;font-size:10px;color:' +
          m.color +
  ...
  if (!html && q.length >= 2) {
    html =
      '<div style="padding:16px;text-align:center;font-family:Cormorant Garamond,serif;font-style:italic;color:rgba(201,168,76,.3);font-size:12px;">Aucun résultat — essayez un autre terme</div>';
  }
  box.innerHTML = html;
...
function analyzeHadith(txt) {
  ...
  var steps = [
    {
      ar: "بِسْمِ اللَّهِ",
      fr: "INITIALISATION AL MIZAN",
      desc: "Ouverture des registres de la science du Hadith.",
    },
    {
      ar: "تَخْرِيجُ الحَدِيث",
      fr: "AT-TAKHRIJ — EXTRACTION DES SOURCES",
      desc: "Recherche du hadith dans les recueils originaux (Bukhari, Muslim, Abu Dawud, At-Tirmidhi, etc.).",
    },
    {
      ar: "دِرَايَةُ الرِّجَال",
      fr: "DIRAYAT AR-RIJAL — LES TRANSMETTEURS",
      desc: "Analyse de la biographie et de la fiabilite de chaque transmetteur de la chaine.",
    },
    {
      ar: "عِلَلُ الإِسْنَاد",
      fr: "ILAL AL-ISNAD — LES DEFAUTS CACHES",
      desc: "Recherche stricte des coupures, inversions ou anomalies dans la chaine de transmission.",
    },
    {
      ar: "الجَرْحُ وَالتَّعْدِيل",
      fr: "AL-JARH WA AT-TADIL",
      desc: "Application des verdicts des Imams Ahmad, Al-Bukhari, Ibn Main, An-Nasai sur les narrateurs.",
    },
    {
      ar: "الحُكْمُ النِّهَائِيّ",
      fr: "AL-HUKM — VERDICT FINAL",
      desc: "Verdict definitif selon les regles de la science du Hadith.",
    },
  ];
  ...
  steps.forEach(function (s, i) {
    ...
    d.id = "step-" + i;
    d.innerHTML =
      '<div class="step-dot"></div><div><p class="step-ar">' +
      s.ar +
      '</p><p class="step-fr">' +
      s.fr +
      '</p><p class="step-desc">' +
  ...
  var localResult =
    isExactArabic && window.MizanDB && window.MizanDB.isLoaded()
      ? window.MizanDB.searchForApp(txt)
      : null;
  ...
  if (localResult && localResult.grade) {
    ...
    var slD = document.getElementById("steps-list");
    slD.innerHTML = "";
    var stepsD = [
      {
        ar: "بِسْمِ اللَّهِ",
...
var MZ_ICONS = {
  ...
  check:
    '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>',
  cross:
    '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>',
};
...
var MZ_LABELS = {
  ...
  MAWDU: {
    ar: "\u0645\u064E\u0648\u0636\u064F\u0648\u0639",
    fr: "REJET\u00c9 \u2014 CE N'EST PAS UN HADITH (MAWDU')",
  },
  TAWAQQUF: {
    ar: "\u062a\u064e\u0648\u064e\u0642\u064f\u0651\u0641",
    fr: "TAWAQQUF \u2014 VERDICT SUSPENDU",
  },
  INCONNU: {
    ar: "\u063a\u064a\u0631 \u0645\u062d\u062f\u062f",
    fr: "\u26a0\ufe0f DA'IF \u2014 STATUT NON CONFIRM\u00c9 (CONSULTER AL-ALBANI)",
  },
};
var MZ_COLORS = {
  SAHIH: "#22c55e",
...
var _MZ_TAWAQQUF_REASONS = {
  terme_protege_detecte:
    "\uD83D\uDD34 Terme prot\u00e9g\u00e9 d\u00e9tect\u00e9 (Bid\u02bfah ou Ta\u02bcw\u012bl interdit)",
...
function mzToggleAcc(el) {
  ...
  if (card) {
    card.querySelectorAll(".mz-acc-body").forEach(function (b) {
      b.classList.remove("mz-acc-open");
    });
    card.querySelectorAll(".mz-acc-head").forEach(function (h) {
      h.classList.remove("mz-acc-active");
    });
  }
  if (!isOpen) {
    body.classList.add("mz-acc-open");
    el.classList.add("mz-acc-active");
  }
}
...
function _mzVerdict(gradeKey, gradeRaw, extra) {
  ...
  if (gradeKey === "TAWAQQUF") {
    ...
    if (reasons.length) {
      ...
      reasons.forEach(function (r) {
        ...
        reasonsHtml +=
          '<li style="font-family:Cormorant Garamond,serif;font-size:12px;color:rgba(167,139,250,.85);line-height:1.7;margin-bottom:2px;">' +
          label +
          "</li>";
      });
      reasonsHtml += "</ul>";
    ...
    if (protectedTerms.length) {
      protectedHtml =
        '<div style="margin-top:10px;padding:8px 12px;background:rgba(167,139,250,.04);border:1px solid rgba(167,139,250,.15);border-radius:8px;">';
      protectedHtml +=
        '<p style="font-family:Cinzel,serif;font-size:7px;letter-spacing:.18em;color:rgba(167,139,250,.5);margin:0 0 6px;">' +
      ...
      protectedTerms.forEach(function (t) {
        ...
        protectedHtml +=
          ...
          icon +
          "</span>" +
          '<span style="font-family:Scheherazade New,serif;font-size:13px;color:rgba(167,139,250,.9);">' +
          (t.term || "") +
          "</span>" +
          (t.note_fr
            ? '<span style="font-family:Cormorant Garamond,serif;font-style:italic;font-size:10px;color:rgba(167,139,250,.5);margin-left:4px;">— ' +
              t.note_fr +
              "</span>"
            : "") +
          "</div>";
    ...
    return (
      '<div class="mz-verdict v-TAWAQQUF" style="background:' +
      bg +
      ";border:1px solid " +
      bd +
      ';flex-direction:column;align-items:stretch;padding:20px;">' +
      '<div style="display:flex;align-items:center;gap:14px;margin-bottom:14px;">' +
      '<div class="mz-verdict-icon" style="border-color:' +
      col +
      "44;background:" +
      bg +
      ";color:" +
      ...
      "</div>" +
      '<div style="flex:1;">' +
      '<span style="font-family:Cinzel,serif;font-size:5.5px;letter-spacing:.55em;color:' +
      col +
      ';opacity:.7;display:block;margin-bottom:4px;">VERDICT \u2014 SUSPENDU</span>' +
      '<div style="display:flex;align-items:center;gap:10px;">' +
      '<span style="font-family:Scheherazade New,serif;font-size:28px;font-weight:700;color:' +
      col +
      ";text-shadow:0 0 20px " +
      col +
      '44;">\u062a\u064e\u0648\u064e\u0642\u064f\u0651\u0641</span>' +
      '<span style="font-family:Cinzel,serif;font-size:16px;font-weight:900;letter-spacing:.06em;color:' +
      col +
      ';">TAWAQQUF</span>' +
      "</div></div></div>" +
      ...
      '<p style="font-family:Cinzel,serif;font-size:9px;font-weight:700;letter-spacing:.1em;color:' +
      col +
      ';margin:0 0 4px;">' +
  ...
  var noteTxt = isSain
    ...
    : "Note scientifique : Cette cha\u00eene sp\u00e9cifique est class\u00e9e " +
      noteGrade +
      " par le muhaddith source. Consultez les voies de renfort (Mutaba\u02bfat) ci-dessous.";

  return (
    `<div class="mz-verdict ${tg.cssClass}" style="background:${tg.colorBg};border:1px solid ${tg.colorBd};flex-direction:column;align-items:stretch;padding:20px;">` +
...
function _mzAcc(title, titleAr, dotColor, content, startOpen) {
  ...
  return (
    '<div class="mz-acc"><button class="mz-acc-head' +
    (startOpen ? " mz-acc-active" : "") +
    '" onclick="mzToggleAcc(this)"><span class="mz-acc-dot" style="background:' +
    dotColor +
    ";box-shadow:0 0 6px " +
    dotColor +
    ';"></span><span class="mz-acc-title" style="color:' +
    dotColor +
    ';">' +
    title +
    (titleAr ? '<span class="mz-acc-title-ar">' + titleAr + "</span>" : "") +
    "</span>" +
    MZ_ICONS.chev +
    '</button><div class="mz-acc-body' +
    (startOpen ? " mz-acc-open" : "") +
    '"><div class="mz-acc-inner">' +
    content +
    "</div></div></div>"
  );
}
function _mzActions(url) {
  return (
    '<div class="mz-actions"><a href="' +
    (url || "https://dorar.net") +
    '" target="_blank"><svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg><span>VERIFIER</span></a><a href="https://islamqa.info/fr" target="_blank"><svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg><span>SAVANT</span></a></div>'
  );
}
...
function _mzIsnadFallback(msg) {
  ...
  p1.style.cssText =
    "font-family:Cinzel,serif;font-size:6.5px;letter-spacing:.25em;color:rgba(201,168,76,.35);margin-bottom:8px;";
  p1.textContent = "ZONE 2 \u2014 SILSILAT AL-ISN\u0100D";
  var p2 = document.createElement("p");
  p2.style.cssText =
    "font-family:Cormorant Garamond,serif;font-style:italic;font-size:13px;color:rgba(201,168,76,.35);line-height:1.7;";
...
function _mzIsnadFromPipe(isnadChain, grade) {
  ...
  if (typeof isnadChain !== "string") {
    try {
      isnadChain = String(isnadChain);
    } catch (_) {
      return _mzIsnadFallback();
    }
  }
  ...
  if (!window._mzBb_loaded) {
    console.log(
      "%c \u2705 M\u00eezan v23.0 : Backbone Vertical charg\u00e9",
      "color:#00ff00;font-weight:bold;",
    );
  ...
  function _nodeColor(n) {
    ...
    if (_IMAMS_RE.test(n.nom)) return { dot: "#22c55e", name: "#86efac", cls: "" };
    if (
      /kadhdhab|\u0643\u0630\u0627\u0628|matruk|\u0645\u062a\u0631\u0648\u0643|munkar|\u0645\u0646\u0643\u0631|wad|\u0648\u0627\u0636/i.test(
        combo,
      )
    )
      return { dot: "#ef4444", name: "#fca5a5", cls: "" };
    if (
      /da.if|\u0636\u0639\u064a\u0641|layyin|\u0644\u064a\u0646|majhul|\u0645\u062c\u0647\u0648\u0644|mudallis|\u0645\u062f\u0644\u0633/i.test(
        combo,
      )
    )
  ...
  try {
    ...
    h +=
      '<p class="mzBb-psub">LE PROPH\u00c8TE MOHAMED \uFDFA \u2014 SOURCE DE LA R\u00c9V\u00c9LATION</p>';
    h += "</div>";
    ...
    dynNodes.forEach(function (n, idx) {
      ...
      h +=
        '<div class="mzBb-dot" style="border-color:' +
        vc.dot +
        ";background:" +
        vc.dot +
        "18;--mzBb-glow:" +
      ...
      h +=
        ...
        dl +
        's;" onclick="window.mzOpenIsnadPanel(\'' +
        _esc(n.nom) +
        "','" +
        _esc(n.titre) +
        "','" +
    ...
    h += "</div>";
    h +=
      '<p class="mzBb-foot">SILSILAT AL-ISN\u0100D \u2014 CHA\u00ceNE DOR\u00c9E ININTERROMPUE \u2014 MANHAJ SALAF</p>';
    h += "</div>";
    return h;
  } catch (err) {
    console.error("[Mizan v23] _mzIsnadFromPipe CRASH:", err);
    return (
      '<div id="mz-isnad-container" style="min-height:40px;padding:14px;background:rgba(255,0,0,.06);border:1px solid rgba(255,0,0,.25);border-radius:10px;"><p style="font-family:Cinzel,serif;font-size:8px;color:#ff6b6b;">\u26a0 ERREUR ZONE 2</p><p style="font-size:12px;color:rgba(255,150,150,.8);">' +
      err.message +
      "</p></div>"
    );
  }
...
function _mzIsnadChain(jarhHtml, grade) {
  ...
  if (!window.mzC4Tog) {
    window.mzC4Tog = function (id) {
      ...
      document.querySelectorAll(".mzC4-vis").forEach(function (x) {
        x.classList.remove("mzC4-vis");
      });
      document.querySelectorAll(".mzC4-open").forEach(function (x) {
        x.classList.remove("mzC4-open");
      });
      if (!op) {
        p.classList.add("mzC4-vis");
        r.classList.add("mzC4-open");
      }
    };
  }
  ...
  function stripHonor(s) {
    return s
      .replace(/<[^>]+>/g, "")
      .replace(
        /\u0635\u0644[\u0651\u064e\u0650]?\u0649?\s*\u0627\u0644\u0644[\u0651\u064e]?\u0647\s*\u0639\u0644\u064a\u0647[\s\u0650]?\s*\u0648\u0633\u0644[\u0651\u064e]?\u0645/g,
        "",
      )
  ...
  ═══════════════════════════════════════════════════════════ */
  var spanRe =
    /<span[^>]*style\s*=\s*["'][^"']*color:\s*([^;"']+)[^"']*["'][^>]*>([\s\S]*?)<\/span>/gi;
  var nodes = [],
    mm;
  ...
  /* Fallback: bold names */
  if (!nodes.length) {
    var bRe = /<(?:strong|b)>([\s\S]*?)<\/(?:strong|b)>/gi,
      bm;
  ...
  if (!nodes.length) {
    ...
    if (arM)
      arM.slice(0, 8).forEach(function (x) {
        var c = x.trim();
  ...
  if (!nodes.length) {
    ...
    return clt.length < 3
      ? ""
      : '<p style="font-size:15px;line-height:1.9;color:rgba(210,200,180,.78);padding:12px 0;">' +
          jarhHtml +
          "</p>";
  }
  ...
  var compRe =
    /bukhari|muslim|tirmidhi|nasa.i|dawud|majah|ahmad.*hanbal|darimi|bayhaqi|\u0628\u062e\u0627\u0631\u064a|\u0645\u0633\u0644\u0645|\u062a\u0631\u0645\u0630\u064a/i;
  var nabiRe =
    /proph|muhammad|\u0631\u0633\u0648\u0644|\u0645\u062d\u0645\u062f|\u0646\u0628\u064a/i;
  if (
    compRe.test(nodes[0].name) ||
    (nodes.length > 1 && nodes[nodes.length - 1].type === "sahabi")
  ) {
    nodes.reverse();
  }
  for (var ci = nodes.length - 1; ci >= 0; ci--) {
    if (compRe.test(nodes[ci].name)) {
      nodes[ci].type = "compiler";
      break;
    }
  }
  /* Remove Nabi from nodes (gets orb) */
  for (var ni = nodes.length - 1; ni >= 0; ni--) {
    if (nabiRe.test(nodes[ni].name)) {
      nodes.splice(ni, 1);
    }
  }
  ...
  var VIS = {
    compiler: {
      m: "Sama\u2019",
      mTip: "Audition directe du Shaykh \u2014 le plus haut degr\u00e9 de r\u00e9ception.",
      rl: "Compilateur (Mu\u1e25addith)",
      st: "A compil\u00e9 ce hadith dans son recueil.",
      bd: "border-[#d4af37]",
      db: "border-[#d4af37]",
      mc: "text-[#d4af37]",
      nc: "text-[#d4af37]",
    ...
    thiqah: {
      m: "\u1e24addathan\u0101",
      mTip: "\u00ab Il nous a rapport\u00e9 \u00bb \u2014 r\u00e9ception orale directe.",
      rl: "R\u0101w\u012b (Transmetteur)",
      st: "Thiqah \u2014 digne de confiance.",
      bd: "border-[#d4af37]",
      db: "border-[#d4af37]",
      mc: "text-green-500",
      nc: "text-white",
      mb: "bg-[#0a1a0f]",
      mr: "border-green-600/30",
      dB: "bg-black",
      dT: "text-[#d4af37]",
    },
    sahabi: {
      m: "\u2018An",
      mTip: "\u00ab D\u2019apr\u00e8s \u00bb \u2014 le Compagnon rapporte du Proph\u00e8te \uFDFA.",
      rl: "\u1e62a\u1e25\u0101b\u012b (Compagnon)",
      st: "\u2018Ad\u016bl par Ijm\u0101\u2019.",
      bd: "border-yellow-500",
      db: "border-yellow-500",
      mc: "text-yellow-500",
      nc: "text-yellow-400",
      mb: "bg-[#1a1400]",
      mr: "border-yellow-500/30",
      dB: "bg-black",
      dT: "text-yellow-400",
    },
    kadhdhab: {
      m: "Inqi\u1e6d\u0101\u2019",
      mTip: "Rupture de cha\u00eene \u2014 transmission non \u00e9tablie.",
      rl: "R\u0101w\u012b Matr\u016bk",
      st: "Rejet\u00e9 par les Imams du Jar\u1e25.",
      bd: "border-red-500",
      db: "border-red-500",
      mc: "text-red-500",
      nc: "text-red-400",
      mb: "bg-[#1a0a0a]",
      mr: "border-red-500/30",
      dB: "bg-red-950",
      dT: "text-red-500",
    },
  };
  ...
  var KB = [
    {
      ...
      epoch: "9e s. \u2014 Bukhara",
      badge: "s",
      bt: "Am\u012br al-Mu\u2019min\u012bn fil \u1e24ad\u012bth",
    },
    {
      re: /muslim|\u0645\u0633\u0644\u0645/i,
      title: "Compilateur",
    ...
    {
      re: /ahmad.*hanbal|\u0623\u062d\u0645\u062f\s*\u0628\u0646\s*\u062d\u0646\u0628\u0644/i,
      title: "Im\u0101m",
      epoch: "9e s. \u2014 Bagdad",
      badge: "g",
      bt: "Im\u0101m Ahl as-Sunnah",
    },
    {
      re: /malik|\u0645\u0627\u0644\u0643\s*\u0628\u0646\s*\u0623\u0646\u0633/i,
      title: "Im\u0101m",
      epoch: "8e s. \u2014 M\u00e9dine",
      badge: "g",
      bt: "Im\u0101m D\u0101r al-Hijrah",
    },
    {
      re: /nafi|\u0646\u0627\u0641\u0639/i,
      title: "T\u0101bi\u2019\u012b",
    ...
    {
      re: /ibn\s*['\u2018]?umar|\u0639\u0628\u062f\u0627\u0644\u0644\u0647\s*\u0628\u0646\s*\u0639\u0645\u0631/i,
      title: "\u1e62a\u1e25\u0101b\u012b",
      epoch: "7e s. \u2014 M\u00e9dine",
      badge: "g",
      bt: "\u2018Ad\u016bl",
    ...
    {
      re: /abu\s*hurayra|\u0623\u0628\u0648\s*\u0647\u0631\u064a\u0631/i,
      title: "\u1e62a\u1e25\u0101b\u012b",
      epoch: "7e s. \u2014 M\u00e9dine",
      badge: "g",
      bt: "\u2018Ad\u016bl \u2014 5 374 a\u1e25\u0101d\u012bth",
    },
    {
      re: /anas|\u0623\u0646\u0633\s*\u0628\u0646\s*\u0645\u0627\u0644\u0643/i,
      title: "\u1e62a\u1e25\u0101b\u012b",
      epoch: "7e\u20138e s. \u2014 Basra",
      badge: "g",
      bt: "\u2018Ad\u016bl \u2014 Kh\u0101dim an-Nab\u012b",
    },
    {
      re: /aisha|\u0639\u0627\u0626\u0634\u0629/i,
      title: "\u1e62a\u1e25\u0101biyyah",
      epoch: "7e s. \u2014 M\u00e9dine",
      badge: "g",
      bt: "\u2018Ad\u016blah \u2014 Umm al-Mu\u2019min\u012bn",
    },
    {
      re: /umar\s*(ibn|bin)?\s*al.?khattab|\u0639\u0645\u0631\s*\u0628\u0646\s*\u0627\u0644\u062e\u0637\u0627\u0628/i,
      title: "\u1e62a\u1e25\u0101b\u012b",
      epoch: "7e s. \u2014 M\u00e9dine",
      badge: "g",
      bt: "\u2018Ad\u016bl \u2014 Al-F\u0101r\u016bq",
    ...
    {
      re: /tinnisi|yusuf.*abd/i,
      title: "T\u0101bi\u2019 at-T\u0101bi\u2019\u012bn",
      epoch: "9e s. \u2014 Tinnis",
      badge: "s",
      bt: "Thiqah Mutqin",
    },
    {
      re: /shu\u2019ba|shu.ba|\u0634\u0639\u0628\u0629/i,
      title: "T\u0101bi\u2019\u012b",
      epoch: "8e s. \u2014 Basra",
      badge: "g",
      bt: "Am\u012br al-Mu\u2019min\u012bn fil \u1e24ad\u012bth",
    ...
    {
      re: /sufyan.*thawri|\u0633\u0641\u064a\u0627\u0646\s*\u0627\u0644\u062b\u0648\u0631\u064a/i,
      title: "T\u0101bi\u2019\u012b",
      epoch: "8e s. \u2014 Kufa",
      badge: "s",
      bt: "Thiqah \u1e24\u0101fi\u1e93",
    ...
    {
      re: /hasan\s*basri|\u0627\u0644\u062d\u0633\u0646\s*\u0627\u0644\u0628\u0635\u0631\u064a/i,
      title: "T\u0101bi\u2019\u012b",
      epoch: "8e s. \u2014 Basra",
      badge: "s",
      bt: "Thiqah",
  ...
  var lnG = isSahih
    ? "linear-gradient(to bottom,#d4af37,#22c55e 60%,#15803d)"
    : "linear-gradient(to bottom,#d4af37,#f59e0b 40%,#ef4444 80%)";
  var lnS = isSahih ? "0 0 15px rgba(212,175,55,.4)" : "0 0 15px rgba(239,68,68,.3)";
  var chv =
    '<svg class="mzC4-chev" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>';
  var arrSvg =
    '<div class="mzC4-arr"><svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><polyline points="19 12 12 19 5 12"/></svg></div>';
  ...
  h +=
    '<span class="mzC4-epoch" style="display:inline-block;margin-bottom:8px;">7e SI\u00c8CLE \u2014 \u00c8RE PROPH\u00c9TIQUE</span>';
  h += '<div class="mzC4-orb"><span>\uFDFA</span></div>';
  h +=
    '<h3 class="text-xl font-black text-[#d4af37] mb-1" style="font-family:\'Scheherazade New\',serif;">Le Proph\u00e8te Muhammad \uFDFA</h3>';
  h +=
    '<p class="text-[9px] text-[#d4af37]/50 uppercase tracking-[0.25em] mb-2" style="font-family:Cinzel,serif;">SOURCE DE LA R\u00c9V\u00c9LATION \u2014 MA\u2018S\u016aM</p>';
  h +=
    '<p class="text-[11px] text-gray-500 italic max-w-xs mx-auto" style="font-family:\'Cormorant Garamond\',serif;line-height:1.6;">';
  ...
  h +=
    '<div class="absolute left-1/2 -translate-x-1/2 w-1.5 h-full z-0 rounded-full" style="background:' +
    lnG +
    ";box-shadow:" +
    lnS +
    ';"></div>';
  ...
  nodes.forEach(function (n, i) {
    ...
    h +=
      '<div class="flex items-center justify-center gap-6 cursor-pointer" onclick="mzC4Tog(\'' +
      nid +
      '\')" style="animation:mzC4In .5s ease both;animation-delay:' +
      dl +
      's;">';

    /* LEFT: Tahammul + Tooltip */
    h +=
      '<div class="w-5/12 flex justify-end"><div class="mzC4-mech ' +
      vi.mb +
      " border " +
      vi.mr +
      ' px-4 py-2.5 rounded-lg text-right max-w-[220px] shadow-lg">';
    h +=
      '<span class="text-[8px] text-gray-500 uppercase tracking-widest block mb-1" style="font-family:Cinzel,serif;">Mode de r\u00e9ception</span>';
    h +=
    ...
    /* DOT */
    h +=
      '<div class="mzC4-dt absolute left-1/2 transform -translate-x-1/2 w-11 h-11 rounded-full ' +
      vi.dB +
      " border-4 " +
      vi.db +
      ' flex items-center justify-center z-20 shadow-[0_0_20px_rgba(0,0,0,1)]" style="transition:box-shadow .3s;">';
    h += '<span class="' + vi.dT + ' text-xs font-bold">' + (i + 1) + "</span></div>";

    /* RIGHT: Name (TITLE) + Genealogy + Badge + Epoch */
    h +=
      '<div class="w-5/12 flex justify-start"><div class="mzC4-card bg-[#111] p-4 rounded-xl border-l-4 ' +
      vi.bd +
    ...
    h +=
      ...
      vi.nc +
      ' leading-tight" style="font-family:\'Scheherazade New\',serif;">' +
      n.name +
      "</h3>";
    h += chv + "</div>";
    ...
    /* Genealogy subtitle */
    h +=
      '<p class="text-[9px] text-gray-500 uppercase tracking-widest mb-2" style="font-family:Cinzel,serif;">' +
      genealogy +
      "</p>";
    ...
    h +=
      '<div class="bg-black/50 p-2.5 rounded border border-gray-800/50 border-l-2 ' + vi.bd + '">';
    h +=
      '<p class="text-[11px] text-gray-400 italic" style="font-family:\'Cormorant Garamond\',serif;line-height:1.6;">\u00ab ' +
      vi.st +
      " \u00bb</p>";
    h += "</div>";
    ...
    h +=
      '<div style="padding:10px 14px;border-radius:8px;background:rgba(139,92,246,.04);border:1px solid rgba(139,92,246,.12);">';
    h +=
      '<p style="font-family:Cinzel,serif;font-size:7px;font-weight:700;letter-spacing:.15em;color:rgba(139,92,246,.5);margin-bottom:4px;">TRANSMISSION</p>';
    h += '<p class="mzC4-vl">';
    if (i === 0) h += "A re\u00e7u du <strong>Proph\u00e8te \uFDFA</strong>";
    else h += "A re\u00e7u de <strong>" + nodes[i - 1].name + "</strong>";
    if (i < nodes.length - 1)
      h += " \u2192 a transmis \u00e0 <strong>" + nodes[i + 1].name + "</strong>";
    ...
    if (t === "kadhdhab") {
      h +=
        '<div style="margin-top:12px;padding:12px 16px;border-radius:8px;background:rgba(239,68,68,.04);border:1px solid rgba(239,68,68,.15);">';
      h +=
        '<p style="font-family:Cinzel,serif;font-size:7px;font-weight:700;letter-spacing:.12em;color:#ef4444;margin-bottom:4px;">\u26a0 \u2018ILLAH \u2014 MAILLON ROMPU</p>';
      h +=
        '<p class="mzC4-vl" style="color:rgba(239,68,68,.7);">al-Jar\u1e25 al-Mufassar muqaddam \u2018ala at-Ta\u2019d\u012bl \u2014 la critique argument\u00e9e pr\u00e9vaut.</p></div>';
    }
  ...
  h += "</div>"; /* /container */
  h +=
    '<p style="font-family:Cinzel,serif;font-size:6px;letter-spacing:.2em;color:rgba(201,168,76,.18);text-align:center;margin-top:10px;">SILSILAT AL-ISN\u0100D \u2014 CASCADE DE PR\u00c9SERVATION</p>';
  return h;
}
...
function _mzScannerFromChain(jarhHtml, grade) {
  ...
  var hasDefect = nodes.some(function (n) {
    return n.defect;
  });
  if (!hasDefect) return "";
  ...
  nodes.forEach(function (n, i) {
    ...
    if (i > 0) {
      var prevBad = nodes[i - 1].defect,
        curBad = n.defect;
      if (prevBad || curBad) {
        html +=
          '<div class="w-10 h-1 flex-shrink-0" style="background:repeating-linear-gradient(90deg,rgba(239,68,68,.6) 0 4px,transparent 4px 8px);"></div>';
      } else {
        html +=
          '<div class="w-10 h-0.5 flex-shrink-0 bg-gradient-to-r from-green-500/50 to-green-500/20 rounded-full"></div>';
      }
    }
    ...
    if (n.defect) {
      ...
      html +=
        '<div class="flex flex-col items-center flex-shrink-0" style="animation:fadeInUp .35s ease both;animation-delay:' +
        delay +
        's;">';
      /* Avatar + radar ping */
      html +=
        ...
        '<span class="text-red-500 font-bold text-lg" style="font-family:\'Scheherazade New\',serif;">' +
        initial +
        "</span>" +
        "</div>" +
      ...
      /* Nom */
      html +=
        '<span class="text-sm font-bold mt-2 text-red-500" style="font-family:\'Scheherazade New\',serif;">' +
        n.name +
        "</span>";
      /* Badge */
      html +=
        '<span class="text-[8px] uppercase tracking-widest px-3 py-1 rounded bg-red-500/10 border border-red-500/30 text-red-500 mt-1 font-bold" style="font-family:Cinzel,serif;">MATRUK \u2014 REJETE</span>';
      /* Cause */
      html +=
        '<p class="text-[10px] text-red-400/70 mt-1 text-center max-w-[120px] italic" style="font-family:\'Cormorant Garamond\',serif;line-height:1.4;">' +
        getCause(n) +
        "</p>";
      html += "</div>";
    } else {
      ...
      html +=
        ...
        '<span class="text-green-500 font-bold text-lg" style="font-family:\'Scheherazade New\',serif;">' +
        initial +
        "</span>" +
        "</div></div>";
      /* Nom */
      html +=
        '<span class="text-sm font-bold mt-2 text-green-400" style="font-family:\'Scheherazade New\',serif;">' +
        n.name +
        "</span>";
      /* Badge */
      html +=
        '<span class="text-[8px] uppercase tracking-widest px-3 py-1 rounded bg-green-500/10 border border-green-500/30 text-green-500 mt-1 font-bold" style="font-family:Cinzel,serif;">THIQAH</span>';
      html += "</div>";
    }
  });
  ...
  html += "</div></div>";
  html +=
    '<p style="font-family:Cinzel,serif;font-size:6.5px;letter-spacing:.25em;color:rgba(239,68,68,.3);text-align:center;margin-top:6px;">SCANNER DE DEFAUT \u2014 LOCALISATION DE L\u2019ILLAH</p>';
  return html;
}
...
function _mzFormatSanad(sanadHtml) {
  ...
  var CONDITIONS = [
    {
      key: "ITTISAL",
      ar: "\u0627\u062A\u0651\u0650\u0635\u064E\u0627\u0644",
      fr: "Continuite de la chaine",
      icon: "link",
    },
    {
      key: "ADALAT",
      ar: "\u0639\u064E\u062F\u064E\u0627\u0644\u064E\u0629",
      fr: "Probite des transmetteurs",
      icon: "shield",
    },
    {
      key: "DABT",
      ar: "\u0636\u064E\u0628\u0652\u0637",
      fr: "Precision memorielle",
      icon: "brain",
    },
    {
      key: "SHUDHUDH",
      ar: "\u0634\u064F\u0630\u064F\u0648\u0630",
      fr: "Absence d'anomalie",
      icon: "eye",
    },
    {
      key: "ILLAH",
      ar: "\u0639\u0650\u0644\u0651\u064E\u0629",
      fr: "Absence de defaut cache",
      icon: "search",
    },
  ];
  var iconSvgs = {
  ...
  CONDITIONS.forEach(function (c, idx) {
    ...
    html +=
      '<div class="mz-check-row" style="animation:fadeInUp .35s ease both;animation-delay:' +
      idx * 0.08 +
      's;">' +
      ...
      (isFail
        ? '<div style="position:absolute;inset:0;border-radius:7px;border:2px solid #ef4444;animation:tridentPing 1.5s cubic-bezier(0,0,.2,1) infinite;"></div>'
        : "") +
      iconSvgs[c.icon] +
      "</div>" +
      '<div class="mz-check-texts">' +
      '<span class="mz-check-name ' +
      cls +
      '">' +
      c.fr +
      "</span>" +
      '<span class="mz-check-ar">' +
      c.ar +
      "</span>" +
      (detail ? '<p class="mz-check-detail">' + detail + "</p>" : "") +
      "</div>" +
      '<span class="mz-check-badge ' +
      cls +
      '" style="background:' +
      statusBg +
      ";color:" +
...
function _mzFormatAvis(avisHtml) {
  ...
  parts.forEach(function (part) {
    ...
    var savantName = "",
      citation = txt;
    var m1 = txt.match(
      /<(?:strong|b)>([\s\S]*?)<\/(?:strong|b)>\s*[:;\u2014\u2013-]?\s*([\s\S]*)/i,
    );
    if (m1) {
      savantName = m1[1].replace(/<[^>]+>/g, "").trim();
      citation = m1[2].trim() || txt;
    }
    if (!savantName) {
      var m2 = txt.match(/^([A-Z\u0600-\u06FF][^:;\u2014]{2,40})\s*[:;\u2014\u2013]\s*([\s\S]+)/);
      if (m2) {
        savantName = m2[1].replace(/<[^>]+>/g, "").trim();
        citation = m2[2].trim();
      }
    }
  ...
  if (!scholars.length)
    return (
      '<p style="font-size:15px;line-height:1.9;color:rgba(210,200,180,.78);">' + avisHtml + "</p>"
    );
  ...
  /* Barre 'Adalah */
  html +=
    '<div class="mz-autopsy-bar-label"><span>\u2018ADALAH \u2014 Integrite</span><span style="color:' +
    adalahColor +
    ';">' +
    adalahPct +
    "%</span></div>";
  html +=
    '<div class="mz-autopsy-bar"><div class="mz-autopsy-bar-fill" style="width:' +
    adalahPct +
    "%;background:" +
    adalahColor +
    ';"></div></div>';
  /* Barre Dabt */
  html +=
    '<div class="mz-autopsy-bar-label"><span>DABT \u2014 Memoire / Precision</span><span style="color:' +
    dabtColor +
    ';">' +
    dabtPct +
    "%</span></div>";
  ...
  if (isSahih) {
    html +=
      '<div style="margin-top:12px;padding:6px 12px;border-radius:6px;background:rgba(34,197,94,.06);border:1px solid rgba(34,197,94,.15);display:inline-block;">' +
    ...
    html +=
      '<div style="margin-top:12px;padding:6px 12px;border-radius:6px;background:rgba(239,68,68,.06);border:1px solid rgba(239,68,68,.15);display:inline-block;">' +
  ...
  html +=
    '<div class="mz-autopsy-etau-title">L\u2019ETAU DE LA VERITE \u2014 AQWAL AL-A\u2019IMMAH</div>';
  ancientScholars.forEach(function (s, i) {
    html +=
      '<div class="mz-autopsy-quote" style="animation:fadeInUp .35s ease both;animation-delay:' +
      i * 0.1 +
      's;">' +
      '<div class="mz-autopsy-quote-name">' +
      (s.name || "IMAM") +
      "</div>" +
      '<div class="mz-autopsy-quote-text">' +
      s.text +
      "</div>" +
      "</div>";
  });
  ...
  else if (rawLower.indexOf("bin baz") !== -1 || rawLower.indexOf("ibn baz") !== -1)
    conclusionScholar = "Cheikh Bin Baz (rahimahullah)";
  else if (rawLower.indexOf("uthaymin") !== -1)
    conclusionScholar = "Cheikh Ibn Uthaymin (rahimahullah)";
  else conclusionScholar = "Cheikh Al-Albani (rahimahullah)";

  html +=
    '<div class="mz-autopsy-conclusion">' +
    "<p><strong>" +
    conclusionScholar +
    "</strong> n\u2019a fait qu\u2019appliquer fidelement les regles etablies par les Imams du Jarh wa at-Ta\u2019dil " +
...
function _mzFormatAlbani(albaniHtml) {
  ...
  var srcPat = [
    ...
    /(Sahih\s+Al-Jami[^,.)]{0,25})/i,
    /(Da[''\u2019]?if\s+Al-Jami[^,.)]{0,25})/i,
    /(Irwa[''\u2019]?\s+Al-Ghalil[^,.)]{0,25})/i,
    /(Takhrij\s+Mishkat[^,.)]{0,25})/i,
    /([Nn][\u00B0o.]\s*\d{1,5})/,
  ];
  for (var i = 0; i < srcPat.length; i++) {
    var sm = raw.match(srcPat[i]);
    if (sm) {
      refName = sm[1].replace(/<[^>]+>/g, "").trim();
      break;
    }
  }
  ...
  var hukmClass = "",
    hukmLabel = "";
  if (/\u0635\u062D\u064A\u062D|SAHIH|[Aa]uthentique/i.test(raw)) {
    hukmClass = "sahih";
    hukmLabel = "SAHIH";
  } else if (/\u0636\u0639\u064A\u0641|DA.IF|[Ff]aible/i.test(raw)) {
    hukmClass = "daif";
    hukmLabel = "DA'IF";
  } else if (/\u0645\u0648\u0636\u0648\u0639|MAWDU|[Ii]nvent/i.test(raw)) {
    hukmClass = "mawdu";
    hukmLabel = "MAWDU'";
  }
  ...
  if (paras.length <= 1 && raw.length > 200) {
    ...
    if (dot > 0 && dot < raw.length - 10)
      paras = [raw.substring(0, dot + 1), raw.substring(dot + 1)];
    else paras = [raw];
  }
  if (paras.length <= 1) paras = [raw];
  var html =
    '<div class="mz-albani"><div class="mz-albani-head">' +
    '<span class="mz-albani-title">AL-ALBANI<span class="mz-albani-title-ar">\u0627\u0644\u0623\u0644\u0628\u064E\u0627\u0646\u0650\u064A</span></span>' +
    '<span class="mz-albani-ref">' +
    refName +
    "</span></div>" +
    '<div class="mz-albani-body">';
  if (hukmClass)
    html += '<span class="mz-albani-hukm ' + hukmClass + '">VERDICT : ' + hukmLabel + "</span>";
...
function renderAIResult(parsed) {
  if (!parsed || parsed === "fallback" || typeof parsed !== "object") {
    showFallbackResult("");
    return;
  }
  if (!parsed.grade) {
  ...
  var isLocal =
    parsed && (parsed._source_type === "LOCAL" || parsed._source_type === "LOCAL_FALLBACK");
  ...
  if (parsed.ar || parsed.matn_ar) {
    html +=
      '<div class="mz-matn"><div class="mz-matn-ar">' +
      (parsed.ar || parsed.matn_ar) +
      "</div></div>";
  }
  ...
  if (hukmText) {
    html +=
      '<div class="mz-illah"><span class="mz-illah-label">' +
      hukmLabel +
      '</span><p class="mz-illah-text">' +
      hukmText +
      "</p></div>";
  }
  ...
  if (parsed.reference) {
    niv3 +=
      '<div class="mz-takhrij"><span class="mz-takhrij-label">TAKHRIJ \u2014 \u0627\u0644\u0645\u064E\u0631\u062C\u0650\u0639</span><p>' +
      parsed.reference +
      "</p></div>";
  }
  ...
  if (isLocal && parsed.badil) {
    niv3 +=
      '<div class="mz-badil"><span class="mz-badil-label">HADITH AUTHENTIQUE DE REMPLACEMENT</span><p class="mz-badil-ar">' +
      parsed.badil.ar +
      '</p><p class="mz-badil-fr">' +
      parsed.badil.fr +
      '</p><p class="mz-badil-src">' +
      parsed.badil.source +
      "</p></div>";
  }
  ...
  if (hasN3)
    html +=
      '<div class="mz-niv3"><div class="mz-niv3-label">TAHQIQ WA TAKHRIJ \u2014 VERIFICATION ET EXTRACTION</div>' +
      niv3 +
      "</div>";
...
function showFallbackResult(txt) {
  ...
  for (var i = 0; i < HADITHS.length; i++) {
    ...
    if (
      q.length > 5 &&
      (hn.indexOf(q.substring(0, 10)) !== -1 || q.indexOf(hn.substring(0, 10)) !== -1)
    ) {
      found = h;
      break;
    }
  ...
  if (!found)
    found = {
      g: "INCONNU",
      r: "Analyse non disponible",
      e: "Consultez dorar.net ou sunnah.com pour une verification approfondie.",
      l: "https://dorar.net",
    };
  var g = found.g || "INCONNU";
  var html =
    '<div class="mz-card"><div class="mz-niv1">' +
    _mzVerdict(g) +
    "</div>" +
    '<div class="mz-illah"><span class="mz-illah-label">RESULTAT</span><p class="mz-illah-text">' +
    found.e +
    "</p></div>";
  if (found.r)
    html +=
      '<div class="mz-niv3"><div class="mz-takhrij"><span class="mz-takhrij-label">REFERENCE</span><p>' +
      found.r +
      "</p></div></div>";
...
function renderExamples() {
  ...
  if (window.MizanDB && window.MizanDB.isLoaded()) {
    examples = window.MizanDB.getByGrade("SAHIH", 2)
      .concat(window.MizanDB.getByGrade("DAIF", 1))
      .concat(window.MizanDB.getByGrade("MAWDU", 1));
  }
  if (!examples.length) {
    HADITHS.slice(0, 4).forEach(function (h) {
      ...
      b.onclick = function () {
        document.getElementById("hadith-input").value = h.t;
        analyzeHadith(h.t);
      };
      el.appendChild(b);
    });
  ...
  examples.forEach(function (h) {
    ...
    b.className = "example-btn";
    b.innerHTML =
      "<span>" + txt + '</span><span class="eg eg-' + h.grade + '">' + h.grade + "</span>";
    b.onclick = (function (hh) {
      return function () {
        document.getElementById("hadith-input").value = hh.fr || hh.ar || "";
        analyzeHadith(hh.fr || hh.ar || "");
      };
    })(h);
    el.appendChild(b);
  });
...
(function () {
  ...
  PREACHERS.forEach(function (p) {
    if (p && p.cat) {
      var n = p.cat.trim().toLowerCase();
      if (!seenGrp.has(n)) {
        seenGrp.add(n);
        GROUPS.push(p.cat.trim());
      }
    }
  });
  ...
  PREACHERS.forEach(function (p) {
    ...
    if (Array.isArray(p.sources))
      p.sources = p.sources.filter(function (s) {
        return s && s.indexOf("dorar.net") === -1;
      });
    if (p.approfondi && Array.isArray(p.approfondi.sources))
      p.approfondi.sources = p.approfondi.sources.filter(function (s) {
        return s && s.indexOf("dorar.net") === -1;
      });
  });
})();
...
function setFilter(f, btn) {
  currentFilter = f;
  document.querySelectorAll("#view-list .filter-btn").forEach(function (b) {
...
function setRegion(r, btn) {
  currentRegion = r;
  document.querySelectorAll("#region-filters .filter-btn").forEach(function (b) {
    b.classList.remove("active-r");
  });
...
function setAffil(a, btn) {
  currentAffil = a;
  document.querySelectorAll("#affil-filters .filter-btn").forEach(function (b) {
    b.classList.remove("active-a");
  });
...
function matchRegion(p) {
  ...
  if (currentRegion === "france")
    return (
      pays.indexOf("france") !== -1 ||
      pays.indexOf("brest") !== -1 ||
      pays.indexOf("paris") !== -1 ||
      pays.indexOf("lyon") !== -1 ||
      pays.indexOf("marseille") !== -1 ||
      pays.indexOf("roubaix") !== -1 ||
      pays.indexOf("bordeaux") !== -1 ||
      pays.indexOf("lille") !== -1 ||
      pays.indexOf("essonne") !== -1 ||
      pays.indexOf("ivry") !== -1 ||
      pays.indexOf("longjumeau") !== -1
    );
  if (currentRegion === "maghreb")
    return (
      pays.indexOf("algérie") !== -1 ||
      pays.indexOf("maroc") !== -1 ||
      pays.indexOf("tunisie") !== -1 ||
      pays.indexOf("maghreb") !== -1 ||
      pays.indexOf("algeria") !== -1
    );
  if (currentRegion === "khalij")
    return (
      pays.indexOf("arabie") !== -1 ||
      pays.indexOf("saoudite") !== -1 ||
      pays.indexOf("koweït") !== -1 ||
      pays.indexOf("emirats") !== -1 ||
      pays.indexOf("qatar") !== -1 ||
      pays.indexOf("bahreïn") !== -1 ||
      pays.indexOf("yémen") !== -1 ||
      pays.indexOf("oman") !== -1 ||
      pays.indexOf("jordanie") !== -1 ||
      pays.indexOf("syrie") !== -1 ||
      pays.indexOf("egypte") !== -1 ||
      pays.indexOf("égypte") !== -1
    );
  if (currentRegion === "occident")
    return (
      pays.indexOf("angleterre") !== -1 ||
      pays.indexOf("usa") !== -1 ||
      pays.indexOf("suisse") !== -1 ||
      pays.indexOf("canada") !== -1 ||
      pays.indexOf("belgique") !== -1 ||
      pays.indexOf("québec") !== -1 ||
      pays.indexOf("royaume") !== -1 ||
      pays.indexOf("uk") !== -1
    );
  if (currentRegion === "asie")
    return (
      pays.indexOf("inde") !== -1 ||
      pays.indexOf("pakistan") !== -1 ||
      pays.indexOf("zimbabwe") !== -1 ||
      pays.indexOf("nigeria") !== -1 ||
      pays.indexOf("malaysia") !== -1 ||
      pays.indexOf("indonésie") !== -1
    );
  return true;
}
function matchAffil(p) {
  ...
  if (currentAffil === "salafi")
    return (
      af.indexOf("salafi") !== -1 ||
      af.indexOf("salafy") !== -1 ||
      af.indexOf("ahlu sunna") !== -1 ||
      af.indexOf("ahlussunna") !== -1
    );
  if (currentAffil === "ikhwan")
    return (
      af.indexOf("ikhwan") !== -1 ||
      af.indexOf("frères") !== -1 ||
      af.indexOf("frérist") !== -1 ||
      af.indexOf("tabligh") !== -1
    );
  if (currentAffil === "soufi")
    return (
      af.indexOf("soufi") !== -1 ||
      af.indexOf("sufi") !== -1 ||
      af.indexOf("tariqa") !== -1 ||
  ...
  if (currentAffil === "liberal")
    return (
      af.indexOf("libéral") !== -1 ||
      af.indexOf("liberal") !== -1 ||
      af.indexOf("réformiste") !== -1 ||
      af.indexOf("moderniste") !== -1
    );
...
function matchFilter(p) {
  ...
  var v = p.verdict || "";
  if (currentFilter === "imam")
    return v === "IMAM" || v === "THIQAH THIQAH" || v === "THIQAH THIQAH — IMAM";
  if (currentFilter === "fiable") return v === "FIABLE" || v === "THIQAH";
  if (currentFilter === "sadouq") return v === "SADOUQ" || v === "VÉRIDIQUE" || v === "SADÛQ";
  if (currentFilter === "surveiller")
    return (
      v === "À SURVEILLER" ||
      v === "PRUDENCE" ||
      v.indexOf("SURVEILLER") !== -1 ||
      v.indexOf("⚠") !== -1 ||
      v === "NUANCÉ"
    );
  if (currentFilter === "layyin") return v === "LAYYIN" || v === "AFFAIBLI" || v === "LÂYYIN";
  if (currentFilter === "daif") return v === "DAIF" || v === "FAIBLE";
  if (currentFilter === "eviter")
    return v === "À ÉVITER" || v === "MATRUK" || v.indexOf("ÉVITER") !== -1;
  if (currentFilter === "mubtadi")
    return v === "DANGER" || v === "MUBTADI" || v.indexOf("DANGER") !== -1;
...
function renderList() {
  ...
  var allF = PREACHERS.filter(function (p) {
    var ms =
      !s ||
      normalize(p.nomFr).indexOf(s) !== -1 ||
      (p.nomAr && p.nomAr.indexOf(document.getElementById("list-search").value) !== -1);
  ...
  var totalAll = PREACHERS.filter(function (p) {
    return matchFilter(p);
  }).length;
  var filterCounts = {
    all: PREACHERS.filter(function (p) {
      var ms =
        !s ||
        normalize(p.nomFr).indexOf(s) !== -1 ||
        (p.nomAr && p.nomAr.indexOf(document.getElementById("list-search").value) !== -1);
    ...
    imam: allF.filter(function (p) {
      var v = p.verdict || "";
      return v === "IMAM" || v === "THIQAH THIQAH" || v.indexOf("THIQAH THIQAH") !== -1;
    }).length,
    fiable: allF.filter(function (p) {
      var v = p.verdict || "";
      return v === "FIABLE" || v === "THIQAH";
    }).length,
    sadouq: allF.filter(function (p) {
      var v = p.verdict || "";
      return v === "SADOUQ" || v === "VÉRIDIQUE";
    }).length,
    surveiller: allF.filter(function (p) {
      var v = p.verdict || "";
      return (
        v.indexOf("SURVEILLER") !== -1 ||
        v === "PRUDENCE" ||
    ...
    layyin: allF.filter(function (p) {
      var v = p.verdict || "";
      return v === "LAYYIN" || v === "AFFAIBLI";
    }).length,
    daif: allF.filter(function (p) {
      var v = p.verdict || "";
      return v === "DAIF" || v === "FAIBLE";
    }).length,
    eviter: allF.filter(function (p) {
      var v = p.verdict || "";
      return v.indexOf("ÉVITER") !== -1 || v === "MATRUK";
    }).length,
    mubtadi: allF.filter(function (p) {
      var v = p.verdict || "";
      return v === "DANGER" || v === "MUBTADI" || v.indexOf("DANGER") !== -1;
    }).length,
    kadhdhab: allF.filter(function (p) {
      var v = p.verdict || "";
      return v === "KADHDHAB" || v === "MENTEUR";
    }).length,
    majhul: allF.filter(function (p) {
      var v = p.verdict || "";
      return v === "MAJHUL" || v === "INCONNU";
    }).length,
  };
  document.querySelectorAll("#view-list .filter-btn[data-filter]").forEach(function (btn) {
    ...
    if (!countEl) {
      ...
      countEl.className = "f-count";
      countEl.style.cssText =
        "font-family:'Cinzel',serif;font-size:11px;font-weight:900;display:block;line-height:1;margin-top:1px;";
  ...
  GROUPS.forEach(function (grp) {
    ...
    var gc = "#c9a84c",
      gb = "rgba(201,168,76,.07)";
    if (grp === "Mises en garde") {
      gc = "#f87171";
      gb = "rgba(239,68,68,.05)";
    } else if (grp === "Répertoire du Manhaj") {
      gc = "#f59e0b";
      gb = "rgba(245,158,11,.06)";
    } else if (grp === "Savants Majeurs") {
      gc = "#a78bfa";
      gb = "rgba(167,139,250,.06)";
    } else if (grp === "Francophones Fiables") {
      gc = "#34d399";
      gb = "rgba(34,197,94,.06)";
    }

    var th =
      '<div class="group-title"><div class="group-line l" style="background:linear-gradient(to right,transparent,' +
      gc +
      ');"></div>' +
      '<span class="group-label" style="color:' +
      gc +
      ";border-color:" +
      gc +
      "55;background:" +
      ...
      grp.toUpperCase() +
      "</span>" +
      "<span style=\"font-family:'Cinzel',serif;font-size:13px;font-weight:700;color:" +
      gc +
      ";background:" +
      gb +
      ";border:1.5px solid " +
      gc +
      '66;border-radius:20px;padding:2px 11px;letter-spacing:.04em;">' +
      items.length +
      "</span>" +
      '<div class="group-line r" style="background:linear-gradient(to left,transparent,' +
      gc +
      ');"></div></div>';
    ...
    visibleItems.forEach(function (p) {
      var cbg = "rgba(255,255,255,.028)",
        cbo = "rgba(201,168,76,.1)",
        csh = "0 2px 12px rgba(0,60,10,.1)";
      if (p.verdict === "FIABLE") {
        cbg = "#f0faf4";
        cbo = "rgba(34,197,94,.28)";
        csh = "0 2px 14px rgba(34,197,94,.12)";
      } else if (p.verdict === "DANGER") {
        cbg = "#1a0505";
        cbo = "rgba(127,29,29,.5)";
        csh = "0 2px 14px rgba(200,30,30,.12)";
      } else if (p.verdict.indexOf("ÉVITER") !== -1) {
        cbg = "#fff8f8";
        cbo = "rgba(239,68,68,.28)";
      } else if (p.verdict.indexOf("PRUDENCE") !== -1 || p.verdict.indexOf("⚠") !== -1) {
        cbg = "#fffbf0";
        cbo = "rgba(245,158,11,.28)";
      }

      var nom = p.nomFr.replace(/'/g, "\\'");
      var ar = (p.nomAr || "").replace(/'/g, "\\'");
      ch +=
        '<button class="pred-btn" style="background:' +
        cbg +
        ";border:1px solid " +
        cbo +
        ";box-shadow:" +
        csh +
        ';" onclick="showPredLoading(\'' +
        nom +
        "','" +
        ar +
        "',function(){openDetail(" +
        p.id +
        ');})">' +
        '<div style="width:5px;flex-shrink:0;align-self:stretch;border-radius:18px 0 0 18px;background:' +
        p.color +
        ';"></div>' +
        '<div style="flex:1;padding:11px 13px;">' +
        '<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;">' +
        '<div style="flex:1;">' +
        '<p class="pred-name" style="color:' +
        (p.verdict === "DANGER" ? "#fca5a5" : "#e8d490") +
        ';">' +
        p.nomFr +
        "</p>" +
        (p.nomAr && p.nomAr !== "بدون اسم"
          ? '<p class="pred-ar" style="color:' +
            (p.verdict === "DANGER" ? "rgba(252,165,165,.7)" : "#c9a84c") +
            ';">' +
            p.nomAr +
            "</p>"
          : "") +
        '<div class="pred-pays" style="margin-top:3px;"><svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg><span style="color:' +
        (p.verdict === "DANGER" ? "rgba(252,165,165,.5)" : "rgba(100,60,10,.45)") +
        '">' +
        (p.pays || "FRANCOPHONIE") +
        "</span></div>" +
        "</div>" +
        '<div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px;flex-shrink:0;">' +
        "<span style=\"font-family:'Cinzel',serif;font-size:6.5px;font-weight:700;padding:4px 10px;border-radius:99px;letter-spacing:.08em;background:" +
        p.color +
        "22;border:1px solid " +
        p.color +
        "55;color:" +
        ...
        p.color +
        '" stroke-width="1.8" stroke-opacity=".5" width="12" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg>' +
        "</div>" +
        "</div></div></button>";
    });
    ...
    if (items.length > visibleItems.length) {
      ...
      ch +=
        "<button onclick=\"iaSectionLoadMore('" +
        secId +
        "')\" " +
  ...
  if (!total)
    el.innerHTML =
      '<div class="no-results"><div class="ar">لا نتائج</div><p>AUCUN RÉSULTAT</p></div>';
}
...
var currentPreacher = null,
  currentDetTab = "essentiel";
function openDetail(id) {
  try {
    currentPreacher =
      PREACHERS.find(function (p) {
        return p.id === id;
      }) || null;
    if (!currentPreacher) {
      console.error("[Al-Mizan] openDetail id=" + id + " introuvable");
      return;
    }
    ...
    if (badge) {
      ...
      badge.style.background = clr + "20";
      badge.style.border = "1px solid " + clr + "44";
      badge.style.color = clr;
    }
    if (nameEl) nameEl.textContent = p.nomFr || "—";
    if (arEl) arEl.textContent = p.nomAr || "";
    document.querySelectorAll(".det-tab-btn").forEach(function (b) {
    ...
    if (el)
      el.innerHTML =
        '<div style="padding:30px;text-align:center;font-family:Cinzel,serif;color:rgba(201,168,76,.4);font-size:10px;letter-spacing:.2em;">FICHE TEMPORAIREMENT INDISPONIBLE</div>';
    goTo("detail");
  }
}
...
function renderDetContent() {
  ...
  if (!currentPreacher) {
    console.warn("[Al-Mizan] renderDetContent: currentPreacher est null");
    return;
  }
  var p = currentPreacher;
  var d = (p && p.data) || {};
  var el = document.getElementById("det-content");
  if (!el) {
    console.warn("[Al-Mizan] renderDetContent: #det-content introuvable");
    return;
  }
  ...
  if (currentDetTab === "essentiel") {
    ...
    html +=
      ...
      "0d;border:1px solid " +
      ic +
      '30;border-radius:16px;padding:14px 16px;margin-bottom:14px;">';
    html +=
      '<p style="font-family:Cinzel,serif;font-size:7px;letter-spacing:.28em;color:' +
      ic +
      ';opacity:.7;margin-bottom:8px;">⚖️ VERDICT ÉTABLI</p>';
    // verdict_detail — supporte objet {note,detail} OU string directe
    var vd = "";
    try {
      var vdRaw = ((p || {}).evaluations || {}).verdict_detail;
      if (vdRaw)
        vd = typeof vdRaw === "object" ? vdRaw.detail || vdRaw.c || "" : String(vdRaw || "");
      if (!vd) {
        var st2 = (p || {}).steps;
        if (Array.isArray(st2) && st2[6]) vd = st2[6].c || st2[6].detail || "";
      }
      if (!vd) vd = d.pourquoi || d.points || "";
    } catch (e) {
      console.warn("[Al-Mizan] verdict_detail:" + e.message);
    }
    if (vd)
      html +=
        '<p style="font-family:Cormorant Garamond,serif;font-size:15px;color:rgba(232,212,140,.9);line-height:1.65;margin-bottom:10px;">' +
        vd +
        "</p>";
    ...
    if (avs.length) {
      ...
      html +=
        '<p style="font-family:Cinzel,serif;font-size:6.5px;letter-spacing:.22em;color:' +
        ic +
        ';opacity:.6;margin-bottom:8px;">PREUVES — AVIS DES SAVANTS</p>';
      avs.forEach(function (av) {
        ...
        html +=
          '<div style="background:rgba(0,0,0,.2);border-left:2px solid ' +
          ic +
          '60;border-radius:0 8px 8px 0;padding:9px 12px;margin-bottom:8px;">';
        html +=
          '<p style="font-family:Cinzel,serif;font-size:7px;letter-spacing:.15em;color:' +
          ic +
          ';margin-bottom:5px;">' +
          savantNom.toUpperCase() +
          "</p>";
        if (citAr)
          html +=
            '<p style="font-family:Scheherazade New,serif;font-size:15px;color:rgba(212,170,100,.75);direction:rtl;text-align:right;line-height:1.6;margin-bottom:4px;">' +
            citAr +
            "</p>";
        if (citFr)
          html +=
            '<p style="font-family:Cormorant Garamond,serif;font-style:italic;font-size:13.5px;color:rgba(200,180,120,.7);line-height:1.5;">' +
            citFr +
            "</p>";
        if (src)
          html +=
            '<p style="font-family:Cinzel,serif;font-size:6px;letter-spacing:.15em;color:rgba(140,110,50,.5);margin-top:5px;">📚 SOURCE : ' +
            src.toUpperCase() +
            "</p>";
        html += "</div>";
      });
    ...
    function getEv(key, stepIdx) {
      try {
        ...
        if (entry && typeof entry === "object")
          return { note: +entry.note || 0, detail: entry.detail || entry.c || "" };
        var stArr = (p || {}).steps;
        if (Array.isArray(stArr) && stArr[stepIdx])
          return { note: 0, detail: stArr[stepIdx].c || stArr[stepIdx].detail || "" };
        return { note: 0, detail: "" };
      } catch (e) {
        console.warn("[Al-Mizan] getEv(" + key + "):" + e.message);
        return { note: 0, detail: "" };
      }
    }
    var criteria = [
      {
        ...
        icon:
          '<svg viewBox="0 0 24 24" fill="none" stroke="' +
          ic +
          '" stroke-width="1.8" width="18" height="18" stroke-linecap="round"><rect x="4" y="2" width="16" height="20" rx="2"/><line x1="8" y1="8" x2="16" y2="8"/><line x1="8" y1="12" x2="16" y2="12"/><line x1="8" y1="16" x2="12" y2="16"/></svg>',
      ...
      },
      {
        ...
        icon:
          '<svg viewBox="0 0 24 24" fill="none" stroke="' +
          ic +
          '" stroke-width="1.8" width="18" height="18" stroke-linecap="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>',
      ...
      },
      {
        ...
        icon:
          '<svg viewBox="0 0 24 24" fill="none" stroke="' +
          ic +
          '" stroke-width="1.8" width="18" height="18" stroke-linecap="round"><polyline points="20 6 9 17 4 12"/></svg>',
        principe:
          "La tazkiya est l'attestation des savants reconnus. Sans elle, nul ne peut enseigner.",
      ...
      },
      {
        ...
        icon:
          '<svg viewBox="0 0 24 24" fill="none" stroke="' +
          ic +
          '" stroke-width="1.8" width="18" height="18" stroke-linecap="round"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>',
    ...
    criteria.forEach(function (cr) {
      try {
        ...
        html +=
          "<div onclick=\"essOpen('ess-" +
          cr.k +
          '\')" style="background:rgba(0,0,0,.25);border:1px solid ' +
          ic +
          '25;border-radius:14px;padding:13px 12px;cursor:pointer;transition:all .2s;">';
        html +=
          '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">';
        html += cr.icon;
        // Badge affiché si jarh_mufassar (obligatoire) OU si note>0 OU si tazkiyat (toujours visible)
        if (jarh_mufassar || note > 0 || cr.k === "tazkiyat")
          html +=
            '<span style="font-family:Cinzel,serif;font-size:8px;letter-spacing:.1em;color:' +
            noteColor +
            ";background:" +
            noteColor +
            "15;border:1px solid " +
            noteColor +
            '40;border-radius:20px;padding:2px 8px;">' +
            noteLabel +
            "</span>";
        html += "</div>";
        html +=
          '<p style="font-family:Cinzel,serif;font-size:8px;letter-spacing:.18em;color:' +
          ic +
          ';margin-bottom:3px;">' +
          cr.label +
          "</p>";
        html +=
          '<p style="font-family:Scheherazade New,serif;font-size:11px;color:rgba(180,150,80,.5);">' +
          cr.ar +
          "</p>";
        html += "</div>";
      } catch (e) {
        console.warn("[Al-Mizan] critère " + cr.k + " : " + e.message);
      }
    });
    html += "</div>";
    ...
    criteria.forEach(function (cr) {
      ...
      html +=
        ...
        "08;border:1px solid " +
        ic +
        '22;border-radius:14px;padding:14px;margin-bottom:10px;">';
      html +=
        '<p style="font-family:Cinzel,serif;font-size:8px;letter-spacing:.18em;color:' +
        ic +
        ';margin-bottom:10px;">' +
      ...
      if (data.detail)
        html +=
          '<p style="font-family:Cormorant Garamond,serif;font-size:15px;color:rgba(220,200,130,.85);line-height:1.65;margin-bottom:10px;">' +
          data.detail +
          "</p>";
      html +=
        '<div style="background:rgba(0,0,0,.2);border-left:2px solid ' +
        ic +
        '50;padding:8px 11px;border-radius:0 8px 8px 0;">';
      html +=
        '<p style="font-family:Cormorant Garamond,serif;font-style:italic;font-size:12.5px;color:rgba(180,150,80,.6);line-height:1.55;">' +
        cr.ref +
        "</p>";
      html += "</div>";
      html += "</div>";
    ...
    if (jarh_mufassar) {
      ...
      html +=
        '<div style="margin-bottom:14px;border:1px solid rgba(153,27,27,.35);border-radius:16px;overflow:hidden;">';
      html +=
        '<div style="background:rgba(127,29,29,.18);padding:11px 16px;border-bottom:1px solid rgba(153,27,27,.25);display:flex;align-items:center;gap:10px;">';
      html +=
        '<svg viewBox="0 0 24 24" fill="none" stroke="#991b1b" stroke-width="1.8" width="15" height="15" stroke-linecap="round"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>';
      html +=
        '<span style="font-family:Cinzel,serif;font-size:7.5px;font-weight:700;letter-spacing:.25em;color:#991b1b;">DÉTAILS DU JARH MUFASSAR</span>';
      html +=
        '<span style="font-family:Scheherazade New,serif;font-size:12px;color:rgba(153,27,27,.6);margin-left:6px;">الجرح المفسَّر</span>';
      html += "</div>";
      html += '<div style="padding:14px 16px;background:rgba(80,0,0,.08);">';
      html +=
        '<p style="font-family:Cinzel,serif;font-size:6.5px;letter-spacing:.25em;color:rgba(153,27,27,.7);margin-bottom:5px;">▸ LE SAVANT JARIH — الجارح</p>';
      html +=
        '<p style="font-family:Cormorant Garamond,serif;font-size:13.5px;font-weight:600;color:rgba(220,180,100,.9);line-height:1.6;margin-bottom:12px;">' +
        (jarihNom || srcDef) +
        "</p>";
      html +=
        '<p style="font-family:Cinzel,serif;font-size:6.5px;letter-spacing:.25em;color:rgba(153,27,27,.7);margin-bottom:5px;">▸ LA PREUVE — الدَّلِيل</p>';
      ...
      html +=
        '<div style="background:rgba(0,0,0,.25);border-right:3px solid rgba(153,27,27,.5);padding:10px 14px;border-radius:10px 0 0 10px;">';
      html +=
        '<p style="font-family:Cormorant Garamond,serif;font-style:italic;font-size:14px;color:rgba(220,190,110,.85);line-height:1.7;">' +
        (proofText || srcDef) +
        "</p>";
      html += "</div>";
      html += "</div></div>";
    ...
    if (pts)
      html +=
        '<div class="info-card" style="border:1px solid #d4af3718;"><div class="accent" style="background:var(--gold)"></div><div class="info-card-tag" style="background:#f59e0b14;border:1px solid #f59e0b28;"><span style="color:#f59e0b">POINTS CLÉS</span></div><p>"' +
        pts +
        '"</p></div>';
    if (d.protection)
      html +=
        '<div class="info-card" style="border:1px solid ' +
        ic +
        '22;"><div class="accent" style="background:' +
        ic +
        '"></div><div class="info-card-tag"><span>PROTECTION</span></div><p>' +
        d.protection +
        "</p></div>";
    ...
    if (prvs.length) {
      ...
      html +=
        '<p style="font-family:Cinzel,serif;font-size:6.5px;letter-spacing:.25em;color:' +
        ic +
        ';opacity:.55;margin-bottom:8px;">🔗 SOURCES VÉRIFIABLES — ISNAD CLIQUABLE</p>';
      ...
      prvs.forEach(function (pr) {
        ...
        html +=
          '<a href="' +
          url +
          '" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:5px;padding:7px 12px;background:rgba(201,168,76,.06);border:1px solid rgba(201,168,76,.18);border-radius:8px;text-decoration:none;transition:all .2s;">';
        html +=
          '<svg viewBox="0 0 24 24" fill="none" stroke="' +
          ic +
          '" stroke-width="2" width="11" height="11" stroke-linecap="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>';
        html +=
          '<span style="font-family:Cinzel,serif;font-size:7px;letter-spacing:.12em;color:' +
          ic +
          ';">' +
          lbl.toUpperCase() +
          "</span>";
    ...
    html +=
      '<details style="margin-top:16px;border:1px solid rgba(201,168,76,.15);border-radius:14px;overflow:hidden;background:rgba(0,0,0,.15);">';
    html +=
      '<summary style="display:flex;align-items:center;gap:10px;padding:14px 16px;cursor:pointer;list-style:none;-webkit-appearance:none;background:rgba(201,168,76,.04);border-bottom:1px solid rgba(201,168,76,.08);transition:background .3s;">';
    html += '<span style="font-size:18px;">🛡️</span>';
    html += '<div style="flex:1;">';
    html +=
      '<span style="font-family:Cinzel,serif;font-size:8.5px;font-weight:700;letter-spacing:.15em;color:#c9a84c;display:block;">L\'HÉRITAGE DES SALAF : LA VOIE DES 12 GÉANTS</span>';
    html +=
      '<span style="font-family:Scheherazade New,serif;font-size:13px;color:rgba(201,168,76,.45);">سبيلُ عمالقة الأمّة</span>';
    html += "</div>";
    html +=
      '<svg viewBox="0 0 24 24" fill="none" stroke="#c9a84c" stroke-width="2" width="14" height="14" style="opacity:.5;transition:transform .3s;"><polyline points="6 9 12 15 18 9"/></svg>';
    html += "</summary>";
    ...
    var gData = [
      {
        n: "① Imam Abou Hanifa (m.150H)",
        ar: "إذا دعاك صاحب بدعة إلى بدعته فاهرب منه",
        fr: "Si le partisan de l\'innovation t\'appelle vers sa bid\'ah, fuis-le.",
        src: "Al-Lalaka\'i · Sharh Usul I\'tiqad",
      },
      {
        n: "② Imam Malik (m.179H)",
        ar: "لا يُؤخذ العلم من صاحب هوى يدعو إليه",
        fr: "On ne prend pas la science du partisan de la passion qui y appelle.",
        src: "Al-Khatib Al-Baghdadi · Muqaddima Ibn as-Salah",
      },
      {
        n: "③ Imam Ach-Chafi\'i (m.204H)",
        ar: "ما لزم أحدٌ أصحاب البدع إلا فارقه الله",
        fr: "Quiconque accompagne les gens de l\'innovation, Allah l\'abandonne.",
        src: "Al-Bayhaqi · Manaqib Ach-Chafi\'i",
      },
      {
        n: "④ Imam Ahmad (m.241H)",
        ar: "لا تجالس صاحب بدعة فإنه لا يؤول أمره إلى خير",
        fr: "Ne t\'assois pas avec l\'innovateur, car son affaire ne mènera à rien de bon.",
        src: "Ibn Battah · Al-Ibana Al-Kubra",
      },
      {
        n: "⑤ Yahya Ibn Ma\'in (m.233H)",
        ar: "الذبّ عن السنة أفضل من الجهاد في سبيل الله",
        fr: "Défendre la Sunnah est meilleur que le Jihad dans le sentier d\'Allah.",
        src: "Al-Khatib · Tarikh Baghdad",
      },
      {
        n: "⑥ Al-Bukhari (m.256H)",
        ar: "صنّف كتاب الضعفاء الصغير والتاريخ الكبير لنقد الرجال",
        fr: "A compilé le Livre des narrateurs faibles et At-Tarikh Al-Kabir pour classer les hommes.",
        src: "Ad-Du\'afa as-Saghir + At-Tarikh Al-Kabir",
      },
      {
        n: "⑦ Imam Muslim (m.261H)",
        ar: "الواجب أن لا يُروى إلا عمّن عُرفت صحة مخارجه والستارة في ناقليه",
        fr: "Il est obligatoire de ne rapporter que de ceux dont on connaît l\'authenticité et la droiture.",
        src: "Muqaddima Sahih Muslim",
      },
      {
        n: "⑧ Abou Hatim Ar-Razi (m.277H)",
        ar: "أدركنا العلماء في جميع الأمصار فوجدناهم على الإجماع في رد البدع",
        fr: "Nous avons rencontré les savants dans toutes les contrées et les avons trouvés unanimes dans le rejet des innovations.",
        src: "Al-Lalaka\'i · Sharh Usul I\'tiqad",
      },
      {
        n: "⑨ Sufyan Ath-Thawri (m.161H)",
        ar: "من أصغى بسمعه إلى صاحب بدعة خرج من ولاية الله",
        fr: "Quiconque prête l\'oreille à un innovateur sort de la protection d\'Allah.",
        src: "Abou Nu\'aym · Hilyat Al-Awliya\'",
      },
      {
        n: "⑩ Ibn Al-Mubarak (m.181H)",
        ar: "الإسناد من الدين ولولا الإسناد لقال من شاء ما شاء",
        fr: "L\'Isnad fait partie de la religion — sans lui, quiconque dirait ce qu\'il veut.",
        src: "Muqaddima Sahih Muslim",
      },
      {
        n: "⑪ Ibn Taymiyyah (m.728H)",
        ar: "بيان حال أهل البدع واجب على الكفاية بل قد يتعيّن",
        fr: "Clarifier l\'état des innovateurs est une obligation communautaire, voire individuelle.",
        src: "Majmu\' Al-Fatawa (28/231)",
      },
      {
        n: "⑫ Adh-Dhahabi (m.748H)",
        ar: "إذا رأيت المبتدع يقول دعونا من الكتاب والأحاديث فاعلم أنه أبو جهل",
        fr: "Lorsque tu vois l\'innovateur dire « laissez les Hadiths », sache qu\'il est le père de l\'ignorance.",
        src: "Siyar A\'lam An-Nubala\'",
      },
    ];
    gData.forEach(function (g) {
      html +=
        '<div style="margin-bottom:14px;padding:12px 14px;background:rgba(201,168,76,.03);border:1px solid rgba(201,168,76,.08);border-radius:10px;">';
      html +=
        '<p style="font-family:Cinzel,serif;font-size:8px;letter-spacing:.18em;color:rgba(201,168,76,.6);margin-bottom:6px;">' +
        g.n +
        "</p>";
      html +=
        '<p style="font-family:Scheherazade New,serif;font-size:1.15rem;color:rgba(232,201,106,.65);direction:rtl;text-align:right;line-height:1.7;margin-bottom:6px;">' +
        g.ar +
        "</p>";
      html +=
        '<p style="font-family:Cormorant Garamond,serif;font-style:italic;font-size:1rem;color:rgba(220,200,160,.75);line-height:1.65;margin-bottom:4px;">' +
        g.fr +
        "</p>";
      html +=
        '<p style="font-family:Cinzel,serif;font-size:6px;letter-spacing:.15em;color:rgba(201,168,76,.35);">📚 ' +
        g.src +
        "</p>";
      html += "</div>";
    });
    ...
    html +=
      '<div style="padding:12px 14px;background:rgba(201,168,76,.06);border:1px solid rgba(201,168,76,.15);border-radius:10px;text-align:center;">';
    html +=
      '<p style="font-family:Cormorant Garamond,serif;font-size:1rem;color:rgba(232,201,106,.8);line-height:1.65;font-weight:600;">12 montagnes, 14 siècles, un seul principe : la mise en garde est une obligation — pas une invention moderne. Cheikh Rabi\', Cheikh Ibn Baz et Cheikh Al-Albani prolongent cette chaîne en or.</p>';
    html += "</div>";
    ...
    html +=
      "<p style=\"margin-top:12px;font-family:Cormorant Garamond,serif;font-style:italic;font-size:10.5px;color:rgba(201,168,76,.35);line-height:1.5;text-align:center;\">Conformément à la règle de l'Isnad — ce verdict repose sur des preuves sourcées et traçables jusqu'aux savants de Ahl as-Sunnah wal-Jama'ah.</p>";
  }
  // ─── ONGLET EXPERT ──────────────────────────────────────────
  ...
  // ── Boutons bas ─────────────────────────────────────────────
  html +=
    '<div style="display:flex;gap:8px;margin-top:18px;padding-bottom:4px;">' +
  ...
  try {
    ...
    el.innerHTML =
      '<div style="padding:20px;text-align:center;font-family:Cinzel,serif;color:rgba(201,168,76,.4);font-size:11px;letter-spacing:.2em;">INFORMATION NON DISPONIBLE</div>';
  }
}
...
function renderExpert(p) {
  ...
  html +=
    '<div style="background:rgba(0,0,0,.25);border:1px solid rgba(201,168,76,.15);border-radius:14px;padding:12px 14px;margin-bottom:14px;text-align:center;">';
  html +=
    '<p style="font-family:Cinzel,serif;font-size:7px;letter-spacing:.3em;color:rgba(201,168,76,.5);margin-bottom:4px;">SOURCES BRUTES — JARH WA TA\'DIL</p>';
  html +=
    '<p style="font-family:Scheherazade New,serif;font-size:16px;color:rgba(212,170,100,.6);direction:rtl;line-height:1.6;">الجرح المفسَّر مقدَّم على التعديل</p>';
  html += "</div>";
  ...
  if (avs.length) {
    html +=
      '<p style="font-family:Cinzel,serif;font-size:7px;letter-spacing:.28em;color:' +
      ic +
      ';opacity:.7;margin-bottom:10px;">▸ AVIS DES SAVANTS — CITATIONS EXACTES</p>';
    avs.forEach(function (av) {
      ...
      html +=
        '<div style="background:rgba(0,0,0,.3);border:1px solid ' +
        ic +
        '20;border-radius:12px;padding:13px 14px;margin-bottom:10px;">';
      html +=
        '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">';
      html += "<div>";
      html +=
        '<p style="font-family:Cinzel,serif;font-size:8px;letter-spacing:.15em;color:' +
        ic +
        ';margin-bottom:1px;">' +
        nom.toUpperCase() +
        "</p>";
      if (nomAr)
        html +=
          '<p style="font-family:Scheherazade New,serif;font-size:13px;color:rgba(180,150,80,.5);">' +
          nomAr +
          "</p>";
      html += "</div>";
      html +=
        '<span style="font-family:Cinzel,serif;font-size:7px;letter-spacing:.15em;color:' +
        typeColor +
        ";background:" +
        typeColor +
        "15;border:1px solid " +
        typeColor +
        '40;border-radius:20px;padding:3px 9px;">' +
        typeLabel +
        "</span>";
      html += "</div>";
      if (citAr)
        html +=
          '<div style="background:rgba(0,0,0,.2);border-right:3px solid ' +
          ic +
          '60;padding:9px 12px;border-radius:8px 0 0 8px;margin-bottom:8px;">' +
          '<p style="font-family:Scheherazade New,serif;font-size:17px;color:rgba(232,201,106,.8);direction:rtl;text-align:right;line-height:1.7;">' +
          citAr +
          "</p>" +
          "</div>";
      if (citFr)
        html +=
          '<p style="font-family:Cormorant Garamond,serif;font-style:italic;font-size:14px;color:rgba(200,180,120,.75);line-height:1.6;margin-bottom:6px;">' +
          citFr +
          "</p>";
      if (src)
        html +=
          '<p style="font-family:Cinzel,serif;font-size:6.5px;letter-spacing:.18em;color:rgba(140,110,50,.55);">📚 ' +
          src.toUpperCase() +
          "</p>";
      html += "</div>";
    });
    ...
    html +=
      '<div style="background:rgba(0,0,0,.2);border:1px solid rgba(201,168,76,.1);border-radius:12px;padding:20px;text-align:center;margin-bottom:14px;">';
    html +=
      '<p style="font-family:Cormorant Garamond,serif;font-style:italic;font-size:13px;color:rgba(180,150,80,.4);">Aucune citation de savant enregistrée pour ce profil.</p>';
    html += "</div>";
  }
  ...
  if (mh.length) {
    html +=
      '<p style="font-family:Cinzel,serif;font-size:7px;letter-spacing:.28em;color:' +
      ic +
      ';opacity:.7;margin:14px 0 10px;">▸ INHIRAF — POINTS DE DÉVIANCE DOCUMENTÉS</p>';
    mh.forEach(function (pt) {
      var niv = pt.niveau || "orange";
      var nivColor = niv === "rouge" ? "#ef4444" : niv === "orange" ? "#f59e0b" : "#22c55e";
      html +=
        '<div style="background:rgba(0,0,0,.2);border-left:3px solid ' +
        nivColor +
        ';border-radius:0 10px 10px 0;padding:10px 13px;margin-bottom:8px;">';
      html +=
        '<p style="font-family:Cinzel,serif;font-size:7.5px;letter-spacing:.15em;color:' +
        nivColor +
        ';margin-bottom:5px;">' +
        pt.point +
        "</p>";
      html +=
        '<p style="font-family:Cormorant Garamond,serif;font-size:13.5px;color:rgba(200,180,120,.7);line-height:1.6;">' +
        pt.detail +
        "</p>";
      html += "</div>";
    });
  ...
  if (bio.formation || bio.maitres || bio.parcours || bio.affiliations) {
    html +=
      '<p style="font-family:Cinzel,serif;font-size:7px;letter-spacing:.28em;color:' +
      ic +
      ';opacity:.7;margin:14px 0 10px;">▸ BIOGRAPHIE & AFFILIATIONS</p>';
    html +=
      '<div style="background:rgba(0,0,0,.2);border:1px solid ' +
      ic +
      '15;border-radius:12px;padding:13px 14px;">';
    var bioFields = [
    ...
    bioFields.forEach(function (f) {
      if (!f.val) return;
      html +=
        '<div style="margin-bottom:9px;padding-bottom:9px;border-bottom:1px solid rgba(201,168,76,.07);">';
      html +=
        '<p style="font-family:Cinzel,serif;font-size:6.5px;letter-spacing:.2em;color:' +
        ic +
        ';opacity:.55;margin-bottom:3px;">' +
        f.label +
        "</p>";
      html +=
        '<p style="font-family:Cormorant Garamond,serif;font-size:13.5px;color:rgba(200,180,120,.7);line-height:1.55;">' +
        f.val +
        "</p>";
      html += "</div>";
    });
  ...
  if (srcs.length) {
    html +=
      '<div style="margin-top:14px;padding:10px 13px;background:rgba(0,0,0,.15);border-radius:10px;">';
    html +=
      '<p style="font-family:Cinzel,serif;font-size:6.5px;letter-spacing:.22em;color:rgba(201,168,76,.4);margin-bottom:7px;">SOURCES VÉRIFIABLES</p>';
    srcs.forEach(function (s) {
      html +=
        '<p style="font-family:Cormorant Garamond,serif;font-size:12px;color:rgba(140,110,50,.6);margin-bottom:3px;">→ ' +
        s +
        "</p>";
    });
    html += "</div>";
...
function renderApprofondi(p) {
  ...
  // ── Principe en tête ───────────────────────────────────────
  html +=
    '<div style="background:rgba(0,0,0,.2);border:1px solid rgba(201,168,76,.12);border-radius:14px;padding:14px;text-align:center;margin-bottom:14px;">';
  html +=
    '<p style="font-family:Scheherazade New,serif;font-size:19px;color:rgba(212,170,100,.65);direction:rtl;line-height:1.6;margin-bottom:4px;">الجرح المفسَّر مقدَّم على التعديل</p>';
  html +=
    '<p style="font-family:Cormorant Garamond,serif;font-style:italic;font-size:12px;color:rgba(150,120,60,.55);">La critique documentée prime sur la recommandation — Sheikh Rabi Al-Madkhali</p>';
  html += "</div>";
  ...
  html +=
    '<div class="ap-trigger-left"><div class="ap-icon i-scholars"><svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke="currentColor"><path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/></svg></div>';
  html +=
    '<div><span class="ap-title">AVIS DES SAVANTS</span><span class="ap-title-ar">أقوال العلماء في الجرح والتعديل</span></div>';
  if (rawAvs.length) html += '<span class="ap-count">' + rawAvs.length + " avis</span>";
  html +=
    '</div><div class="ap-arrow"><svg viewBox="0 0 24 24" fill="none" stroke-width="2.5" stroke="currentColor"><polyline points="6 9 12 15 18 9"/></svg></div></button>';
  ...
  if (rawAvs.length) {
    rawAvs.forEach(function (av) {
      ...
      html +=
        '<div style="background:rgba(0,0,0,.22);border:1px solid rgba(201,168,76,.1);border-radius:12px;padding:13px;margin-bottom:10px;">';
      // Header savant
      html += '<div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">';
      html +=
        '<div style="width:36px;height:36px;border-radius:50%;background:' +
        ic +
        "20;border:1.5px solid " +
        ic +
        "40;display:flex;align-items:center;justify-content:center;font-family:Scheherazade New,serif;font-size:14px;color:" +
        ic +
        ';flex-shrink:0;">' +
        (nomAr ? nomAr.charAt(1) || nomAr.charAt(0) : nom.charAt(0)) +
        "</div>";
      ...
      if (epoque)
        html +=
          '<p style="font-family:Cormorant Garamond,serif;font-size:11px;color:rgba(180,150,80,.5);">' +
          epoque +
          "</p>";
      html += "</div>";
      html +=
        '<span style="font-family:Cinzel,serif;font-size:6.5px;letter-spacing:.1em;color:' +
        badgeC +
        ";background:" +
        badgeC +
        "18;border:1px solid " +
        badgeC +
        '40;border-radius:20px;padding:3px 8px;flex-shrink:0;">' +
        badgeT +
        "</span>";
      html += "</div>";
      // Citation arabe
      if (citAr)
        html +=
          '<p style="font-family:Scheherazade New,serif;font-size:16px;color:rgba(212,170,100,.75);direction:rtl;text-align:right;line-height:1.7;padding:8px 10px;background:rgba(201,168,76,.05);border-radius:8px;margin-bottom:8px;">' +
          citAr +
          "</p>";
      // Traduction
      if (citFr)
        html +=
          '<p style="font-family:Cormorant Garamond,serif;font-style:italic;font-size:14px;color:rgba(200,180,120,.7);line-height:1.6;">"&nbsp;' +
          citFr +
          '&nbsp;"</p>';
      // Source
      if (src)
        html +=
          '<p style="font-family:Cinzel,serif;font-size:6px;letter-spacing:.15em;color:rgba(140,110,50,.45);margin-top:6px;padding-top:6px;border-top:1px solid rgba(201,168,76,.08);">📚 SOURCE : ' +
          src.toUpperCase() +
          "</p>";
      html += "</div>";
    });
    html +=
      '<div style="padding:10px;text-align:center;"><p style="font-family:Cormorant Garamond,serif;font-style:italic;font-size:12px;color:rgba(120,90,40,.45);line-height:1.5;">"&nbsp;Si dix savants le recommandent et un seul le critique avec preuve, c\'est la critique qui prime.&nbsp;" — Sheikh Rabi Al-Madkhali</p></div>';
  } else {
    html +=
      '<p style="font-family:Cormorant Garamond,serif;font-style:italic;color:rgba(90,62,32,.5);font-size:13px;padding:12px 0;">Aucun avis de savant documenté pour ce profil.</p>';
  }
  html += "</div></div></div>";
  ...
  html +=
    '<div class="ap-trigger-left"><div class="ap-icon i-manhaj"><svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke="currentColor"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg></div>';
  html +=
    '<div><span class="ap-title">ANALYSE DÉTAILLÉE</span><span class="ap-title-ar">التحليل المفصَّل للمنهج</span></div>';
  html +=
    '</div><div class="ap-arrow"><svg viewBox="0 0 24 24" fill="none" stroke-width="2.5" stroke="currentColor"><polyline points="6 9 12 15 18 9"/></svg></div></button>';
  ...
  if (hasCriteria) {
    ...
    critMap.forEach(function (cr) {
      ...
      html +=
        '<div><span style="font-family:Cinzel,serif;font-size:8px;letter-spacing:.15em;color:rgba(200,170,100,.8);">' +
        cr.label +
        '</span> <span style="font-family:Scheherazade New,serif;font-size:11px;color:rgba(180,150,80,.4);">' +
        cr.ar +
        "</span></div>";
      html +=
        '<span style="font-family:Cinzel,serif;font-size:9px;font-weight:700;color:' +
        nc +
        ';">' +
        n +
        "/10</span>";
      html += "</div>";
      html +=
        '<div style="height:4px;background:rgba(255,255,255,.06);border-radius:4px;margin-bottom:6px;"><div style="height:100%;width:' +
        pct +
        "%;background:" +
        nc +
      ...
      if (e.detail)
        html +=
          '<p style="font-family:Cormorant Garamond,serif;font-size:13.5px;color:rgba(200,180,130,.65);line-height:1.55;">' +
          e.detail +
          "</p>";
      html += "</div>";
    });
    ...
    ap.manhaj.forEach(function (m) {
      ...
      html +=
        '<div style="display:flex;gap:10px;margin-bottom:10px;padding-bottom:10px;border-bottom:1px solid rgba(201,168,76,.07);">';
      html +=
        '<div style="width:8px;height:8px;border-radius:50%;background:' +
        nc +
        ';flex-shrink:0;margin-top:5px;"></div>';
      html +=
        '<div><p style="font-family:Cinzel,serif;font-size:7.5px;letter-spacing:.15em;color:' +
        nc +
        ';margin-bottom:3px;">' +
        m.point.toUpperCase() +
        "</p>";
    ...
    html +=
      '<p style="font-family:Cormorant Garamond,serif;font-style:italic;color:rgba(90,62,32,.5);font-size:13px;padding:12px 0;">Analyse du manhaj en cours de compilation.</p>';
  }
  html += "</div></div></div>";
  ...
  html +=
    '<div class="ap-trigger-left"><div class="ap-icon i-books"><svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke="currentColor"><circle cx="12" cy="8" r="4"/><path d="M6 20v-2a4 4 0 0 1 8 0v2"/><path d="M18 20v-2a2 2 0 0 0-2-2h-1"/></svg></div>';
  html +=
    '<div><span class="ap-title">BIOGRAPHIE & ACTIVITÉ</span><span class="ap-title-ar">السِّيرة والنَّشاط</span></div>';
  html +=
    '</div><div class="ap-arrow"><svg viewBox="0 0 24 24" fill="none" stroke-width="2.5" stroke="currentColor"><polyline points="6 9 12 15 18 9"/></svg></div></button>';
    ...
    if (typeof data === "object") {
      ...
      return (
        parts
          ...
          .trim() || "Information non documentée."
      );
    }
    return String(data)
      .replace(/\[object Object\]/g, "")
  ...
  if (bio) {
    ...
    html +=
      '<p style="font-family:Cinzel,serif;font-size:7px;letter-spacing:.22em;color:rgba(180,150,80,.55);margin-bottom:6px;">BIOGRAPHIE</p>';
    html +=
      '<p style="font-family:Cormorant Garamond,serif;font-size:14.5px;color:rgba(220,200,140,.8);line-height:1.65;">' +
      bio +
      "</p>";
    html += "</div>";
  }
  if (act) {
    html += "<div>";
    html +=
      '<p style="font-family:Cinzel,serif;font-size:7px;letter-spacing:.22em;color:rgba(180,150,80,.55);margin-bottom:6px;">ACTIVITÉ</p>';
    html +=
      '<p style="font-family:Cormorant Garamond,serif;font-size:14.5px;color:rgba(220,200,140,.8);line-height:1.65;">' +
      act +
      "</p>";
  ...
  if (p.domaines && p.domaines.length) {
    ...
    p.domaines.forEach(function (d) {
      html +=
        '<span style="font-family:Cinzel,serif;font-size:6.5px;letter-spacing:.12em;color:' +
        ic +
        ";background:" +
        ic +
        "12;border:1px solid " +
        ic +
        '30;border-radius:20px;padding:3px 10px;">' +
        d.toUpperCase() +
        "</span>";
    });
    html += "</div>";
  ...
  if (oeuvres.length) {
    ...
    html +=
      '<div class="ap-trigger-left"><div class="ap-icon i-books"><svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke="currentColor"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg></div>';
    html +=
      '<div><span class="ap-title">ŒUVRES & SOURCES</span><span class="ap-title-ar">المُؤلَّفات والمَصادر</span></div>';
    ...
    oeuvres.forEach(function (o) {
      ...
      if (o.titre_ar)
        html +=
          '<p style="font-family:Scheherazade New,serif;font-size:15px;color:rgba(212,170,100,.75);direction:rtl;text-align:right;margin-bottom:3px;">' +
          o.titre_ar +
          "</p>";
      if (o.titre_fr)
        html +=
          '<p style="font-family:Cormorant Garamond,serif;font-size:14px;color:rgba(200,180,130,.75);">' +
          o.titre_fr +
          "</p>";
      if (o.desc)
        html +=
          '<p style="font-family:Cormorant Garamond,serif;font-style:italic;font-size:12.5px;color:rgba(160,130,70,.55);margin-top:3px;">' +
          o.desc +
          "</p>";
      html += "</div>";
    });
  ...
  if (preuves.length) {
    ...
    html +=
      '<div class="ap-trigger-left"><div class="ap-icon"><svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke="currentColor"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg></div>';
    html +=
      '<div><span class="ap-title">LIENS & PREUVES</span><span class="ap-title-ar">المَصادر والرَّوابط</span></div>';
    ...
    preuves.forEach(function (pr) {
      ...
      html +=
        '<a href="' +
        (pr.u || "#") +
        '" target="_blank" style="display:block;padding:9px 12px;background:rgba(0,0,0,.2);border:1px solid ' +
        ic +
        '20;border-radius:10px;margin-bottom:7px;text-decoration:none;">';
      html +=
        '<p style="font-family:Cinzel,serif;font-size:7.5px;letter-spacing:.15em;color:' +
        ic +
        ';">🔗 ' +
...
function renderMythes() {
  var el = document.getElementById("myths-list");
  el.innerHTML = "";
  // Section 1 : Hadiths faibles/inventés
  var hadithMythes = MYTHES.filter(function (m) {
    return !m.cat || m.cat !== "khurafat";
  });
  var khurafatMythes = MYTHES.filter(function (m) {
    return m.cat === "khurafat";
  });

  if (hadithMythes.length) {
    var hdr1 = document.createElement("div");
    hdr1.style.cssText =
      "display:flex;align-items:center;gap:10px;margin-bottom:14px;padding:12px 16px;background:rgba(239,68,68,.05);border:1px solid rgba(239,68,68,.15);border-radius:12px;";
    hdr1.innerHTML =
      '<span style="font-size:20px;">📖</span><div><span style="font-family:Cinzel,serif;font-size:9px;font-weight:700;letter-spacing:.18em;color:#ef4444;display:block;">AHADITH DA\'IFA & MAWDU\'A</span><span style="font-family:Scheherazade New,serif;font-size:14px;color:rgba(239,68,68,.45);">الأحاديث الضعيفة والموضوعة</span></div>';
    el.appendChild(hdr1);
    hadithMythes.forEach(function (m) {
      var card = document.createElement("div");
      card.className = "myth-card";
      card.innerHTML =
        '<div class="myth-header"><div class="myth-hadith">"' +
        m.hadith +
        '"</div><span class="myth-badge" style="background:' +
        m.color +
        "20;border:1px solid " +
        m.color +
        "44;color:" +
        ...
        '">' +
        m.grade +
        '</span></div><div class="myth-footer"><span class="myth-ref">' +
        m.ref +
        '</span><span class="myth-expand-hint">DÉTAIL →</span></div>';
  ...
  if (khurafatMythes.length) {
    ...
    khurafatMythes.forEach(function (m) {
      ...
      card.innerHTML =
        '<div style="display:flex;align-items:stretch;"><div style="width:3px;flex-shrink:0;background:' +
        m.color +
        ';border-radius:3px 0 0 3px;"></div><div style="flex:1;"><div class="myth-header"><div class="myth-hadith">"' +
        m.hadith +
        '"</div><span class="myth-badge" style="background:' +
        m.color +
        "20;border:1px solid " +
        ...
        '</span></div><div class="myth-footer"><span class="myth-ref">' +
        m.ref +
        '</span><span class="myth-expand-hint">DÉTAIL →</span></div></div></div>';
      card.onclick = function () {
        openMythDetail(m.id);
      };
      el.appendChild(card);
...
function openMythDetail(id) {
  currentMyth = MYTHES.find(function (m) {
    return m.id === id;
  });
  ...
  // Origine
  html +=
    '<div class="myth-explication" style="border:1px solid #d4af3720;"><div class="accent" style="background:var(--gold)"></div>' +
  ...
  // Explication
  html +=
    '<div class="myth-explication"><div class="accent"></div>' +
  ...
  // Erreur
  html +=
    '<div class="myth-explication" style="border:1px solid #ef444428;"><div class="accent" style="background:#ef4444"></div>' +
  ...
  // Correction
  html +=
    '<div class="myth-authentic"><div class="accent"></div>' +
    ...
    '<p class="myth-ar-text">' +
    m.ar_correction +
    "</p>" +
    '<p class="myth-text" style="color:rgba(212,170,100,.72);">' +
    m.correction +
    "</p>" +
    '<p class="myth-source">SOURCE : ' +
    m.source_correction +
    "</p></div>";
  // Disclaimer
  html +=
    '<div class="disclaimer" style="margin-top:8px;"><p>⚠ Fondé sur le Coran, la Sunnah authentique, la voie des 4 Imams (Abou Hanifa, Malik, Ach-Chafi\'i, Ahmad) et confirmé par Al-Albani, Ibn Baz, Ibn Uthaymin et Al-Fawzan.</p></div>';
  el.innerHTML = html;
  goTo("myth-detail");
}
...
function renderFiraq() {
  ...
  FIRAQ.forEach(function (f, idx) {
    ...
    for (var i = 1; i <= 5; i++) {
      niveauBar +=
        '<span style="width:16px;height:4px;background:' +
        (i <= f.niveau ? f.color : "rgba(255,255,255,.06)") +
        ";display:inline-block;margin-right:2px;box-shadow:" +
        (i <= f.niveau ? "0 0 4px " + f.color + "44" : "none") +
        '"></span>';
    }
    var card = document.createElement("div");
    card.style.cssText =
      ...
      f.color +
      "30;border-radius:3px;overflow:hidden;margin-bottom:10px;cursor:pointer;transition:all .3s cubic-bezier(.4,0,.2,1);animation:argCardIn .4s ease both;animation-delay:" +
      idx * 0.06 +
      "s;";
    card.innerHTML =
      ...
      '<p style="font-family:Cinzel,serif;color:#c9a84c;font-size:10.5px;font-weight:700;letter-spacing:.1em;line-height:1.3;margin-bottom:3px;">' +
      f.nom +
      "</p>" +
      '<p style="font-family:Scheherazade New,serif;color:' +
      f.color +
      ';font-size:15px;line-height:1.1;opacity:.7;">' +
      f.ar +
      "</p>" +
      "</div>" +
      '<span style="font-family:Cinzel,serif;font-size:5.5px;font-weight:700;padding:4px 10px;border-radius:2px;background:' +
      f.color +
      "15;border:1px solid " +
      f.color +
      "35;color:" +
      ...
      "</span>" +
      "</div>" +
      '<p style="font-family:Cormorant Garamond,serif;font-style:italic;color:rgba(220,200,160,.5);font-size:12px;line-height:1.6;margin-bottom:8px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">' +
      (f.phraseChoc || f.description).substring(0, 120) +
      "...</p>" +
      ...
      "</div>" +
      "</div>" +
      '<span style="font-family:Cinzel,serif;font-size:6px;letter-spacing:.15em;color:' +
      f.color +
      ';opacity:.5;">OUVRIR ▸</span>' +
      "</div>" +
      "</div>" +
      "</div>";
    card.onmouseenter = function () {
      this.style.borderColor = f.color + "60";
      this.style.background = "#151d2e";
    };
    card.onmouseleave = function () {
      this.style.borderColor = f.color + "30";
      this.style.background = "#111827";
    };
    card.onclick = function () {
      openFiraqDetail(f.id);
    };
    el.appendChild(card);
  });
}
...
function openFiraqDetail(id) {
  currentFiraq = FIRAQ.find(function (f) {
    return f.id === id;
  });
...
function setFiraqTab(tab, btn) {
  firaqTab = tab;
  document.querySelectorAll(".fq-tab-btn").forEach(function (b) {
    b.classList.remove("active");
  });
  btn.classList.add("active");
  renderFiraqDetail();
...
function renderFiraqDetail() {
  ...
  var tabsHtml =
    '<div style="display:flex;background:rgba(9,5,0,.97);border-bottom:1px solid rgba(140,100,30,.18);position:sticky;top:0;z-index:10;">';
  tabs.forEach(function (t) {
    var active = firaqTab === t[0];
    var btn = document.createElement("button");
    btn.style.cssText =
      "flex:1;padding:11px 4px;font-family:Cinzel,serif;font-size:6.5px;font-weight:700;letter-spacing:.18em;background:transparent;border:none;border-bottom:2px solid " +
      (active ? "#c9a84c" : "transparent") +
      ";cursor:pointer;color:" +
      (active ? "#c9a84c" : "rgba(100,60,10,.4)");
    btn.textContent = t[1];
    btn.className = "fq-tab-btn" + (active ? " active" : "");
    (function (tab) {
      btn.onclick = function () {
        setFiraqTab(tab, this);
      };
    })(t[0]);
  ...
  if (firaqTab === "essentiel") {
    ...
    bodyHtml +=
      ...
      f.color +
      "30;border-radius:3px;padding:16px 18px;margin-bottom:12px;border-left:3px solid " +
      f.color +
      ';">' +
      '<p style="font-family:Cormorant Garamond,serif;font-style:italic;color:rgba(232,201,106,.82);font-size:1.1rem;line-height:1.8;">' +
      f.phraseChoc +
      "</p>" +
      "</div>";
    ...
    for (var i = 1; i <= 5; i++) {
      niveauBar +=
        '<span style="width:18px;height:5px;background:' +
        (i <= f.niveau ? f.color : "rgba(255,255,255,.06)") +
        ";display:inline-block;margin-right:3px;box-shadow:" +
        (i <= f.niveau ? "0 0 6px " + f.color + "44" : "none") +
        '"></span>';
    }
    bodyHtml +=
      '<div style="background:#0d1117;border:1px solid rgba(201,168,76,.12);border-radius:3px;padding:12px 16px;margin-bottom:12px;display:flex;align-items:center;gap:14px;">' +
      ...
      '<span style="font-family:Cinzel,serif;font-size:8px;font-weight:700;color:' +
      f.color +
      ';">' +
      f.danger +
      "</span></div>" +
    ...
    bodyHtml +=
      '<div style="background:#0d1117;border:1px solid rgba(201,168,76,.12);border-radius:3px;padding:12px 16px;margin-bottom:12px;border-left:3px solid #c9a84c;">' +
      ...
      '<p style="font-family:Cormorant Garamond,serif;font-style:italic;color:rgba(220,200,160,.7);font-size:1rem;line-height:1.7;">' +
      f.fondateur +
      "</p>" +
      "</div>";
    // Description — NARRATION HISTORIQUE — texte grand et aéré
    bodyHtml +=
      '<div style="background:#111827;border:1px solid rgba(201,168,76,.08);border-radius:3px;padding:16px 18px;margin-bottom:12px;">' +
      ...
      '<p style="font-family:Cormorant Garamond,serif;color:rgba(220,200,160,.75);font-size:1.1rem;line-height:1.8;">' +
      f.description +
      "</p>" +
      "</div>";
    ...
    bodyHtml +=
      ...
      f.color +
      "25;border-radius:3px;padding:14px 16px;margin-bottom:12px;border-left:3px solid " +
      f.color +
      ';">' +
      '<span style="font-family:Cinzel,serif;font-size:6px;letter-spacing:.3em;color:' +
      f.color +
      ';opacity:.75;display:block;margin-bottom:10px;">SIGNES DE RECONNAISSANCE</span>';
    f.signes.forEach(function (s, i) {
      bodyHtml +=
        '<div style="display:flex;align-items:flex-start;gap:10px;padding:7px 0;' +
        (i < f.signes.length - 1 ? "border-bottom:1px solid rgba(255,255,255,.04)" : "") +
        '">' +
        '<span style="width:6px;height:2px;background:' +
        f.color +
        ';flex-shrink:0;margin-top:9px;"></span>' +
        '<p style="font-family:Cormorant Garamond,serif;color:rgba(220,200,160,.65);font-size:1rem;line-height:1.65;">' +
        s +
        "</p>" +
        "</div>";
    });
    ...
    bodyHtml +=
      '<div style="background:#111827;border:1px solid rgba(153,27,27,.2);border-radius:3px;padding:14px 16px;margin-bottom:12px;border-left:3px solid #991b1b;">' +
      ...
      '<p style="font-family:Cormorant Garamond,serif;color:rgba(220,200,160,.75);font-size:1.05rem;line-height:1.8;">' +
      f.hukm_sharh +
      "</p>" +
      "</div>";
    // Présence en France
    bodyHtml +=
      '<div style="background:#0d1117;border:1px solid rgba(99,102,241,.15);border-radius:3px;padding:12px 16px;border-left:3px solid #6366f1;">' +
      ...
      '<p style="font-family:Cormorant Garamond,serif;color:rgba(220,200,160,.65);font-size:1rem;line-height:1.75;">' +
      f.presence_fr +
      "</p>" +
      "</div>";
    ...
    if (f.hajr) {
      bodyHtml +=
        '<div style="background:#111827;border:1px solid rgba(245,158,11,.18);border-radius:3px;padding:14px 16px;margin-top:12px;border-left:3px solid #f59e0b;">' +
        ...
        '<p style="font-family:Cormorant Garamond,serif;color:rgba(220,200,160,.72);font-size:1rem;line-height:1.8;">' +
        f.hajr +
        "</p>" +
        "</div>";
    }
    ...
    if (f.masques && f.masques.length) {
      bodyHtml +=
        '<div style="background:#0d1117;border:1px solid rgba(167,139,250,.15);border-radius:3px;padding:14px 16px;margin-top:12px;border-left:3px solid #a78bfa;">' +
      ...
      f.masques.forEach(function (m, i) {
        bodyHtml +=
          '<div style="display:flex;align-items:flex-start;gap:10px;padding:7px 0;' +
          (i < f.masques.length - 1 ? "border-bottom:1px solid rgba(255,255,255,.04)" : "") +
          '">' +
          '<span style="width:6px;height:2px;background:#a78bfa;flex-shrink:0;margin-top:9px;"></span>' +
          '<p style="font-family:Cormorant Garamond,serif;color:rgba(220,200,160,.68);font-size:1rem;line-height:1.65;">' +
          m +
          "</p>" +
          "</div>";
      });
    ...
    bodyHtml +=
      '<span style="font-family:Cinzel,serif;font-size:6px;letter-spacing:.4em;color:rgba(201,168,76,.35);display:block;margin-bottom:14px;">CITATIONS DES SAVANTS</span>';
    f.citations.forEach(function (c, i) {
      bodyHtml +=
        '<div style="background:#111827;border:1px solid rgba(201,168,76,.12);border-radius:3px;padding:14px 16px;margin-bottom:10px;">' +
        '<div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">' +
        '<div style="width:30px;height:30px;border-radius:2px;background:rgba(201,168,76,.1);border:1px solid rgba(201,168,76,.25);display:flex;align-items:center;justify-content:center;font-family:Scheherazade New,serif;color:#c9a84c;font-size:14px;flex-shrink:0;">' +
        c.savant_ar.charAt(0) +
        "</div>" +
        '<div style="flex:1;"><span style="font-family:Cinzel,serif;font-size:9px;font-weight:700;color:rgba(232,201,106,.85);display:block;">' +
        c.savant +
        "</span>" +
        '<span style="font-family:Cinzel,serif;font-size:5.5px;color:rgba(201,168,76,.35);letter-spacing:.15em;">' +
        c.source +
        "</span></div>" +
        ...
        '<span style="font-family:Scheherazade New,serif;font-size:1.1rem;direction:rtl;text-align:right;display:block;color:rgba(232,201,106,.8);line-height:1.8;margin-bottom:6px;">' +
        c.citation_ar +
        "</span>" +
        '<p style="font-family:Cormorant Garamond,serif;font-style:italic;color:rgba(220,200,160,.6);font-size:1rem;line-height:1.7;">' +
        c.citation_fr +
        "</p>" +
        "</div>" +
        "</div>";
    ...
    bodyHtml +=
      '<div style="background:#0d1117;border:1px solid rgba(34,197,94,.18);border-radius:3px;padding:12px 16px;margin-bottom:10px;border-left:3px solid #22c55e;">' +
    ...
    f.savants.forEach(function (s) {
      bodyHtml +=
        '<div style="display:flex;align-items:center;gap:8px;padding:5px 0;border-bottom:1px solid rgba(34,197,94,.06);">' +
        '<span style="width:6px;height:2px;background:#22c55e;flex-shrink:0;"></span>' +
        '<span style="font-family:Cinzel,serif;font-size:8px;color:rgba(34,197,94,.75);">' +
        s +
        "</span>" +
        "</div>";
    });
    bodyHtml += "</div>";
    bodyHtml +=
      '<div style="background:#111827;border:1px solid rgba(201,168,76,.1);border-radius:3px;padding:12px 16px;">' +
      ...
      '<p style="font-family:Cormorant Garamond,serif;color:rgba(220,200,160,.6);font-size:1rem;line-height:1.75;">' +
      f.refutation +
      "</p>" +
      "</div>";
  } else if (firaqTab === "comment") {
    bodyHtml +=
      '<div style="background:#111827;border:1px solid rgba(245,158,11,.2);border-radius:3px;padding:14px 16px;margin-bottom:12px;border-left:3px solid #f59e0b;">' +
      ...
      '<p style="font-family:Cormorant Garamond,serif;color:rgba(220,200,160,.7);font-size:1.05rem;line-height:1.8;">' +
      f.comment_reconnaitre +
      "</p>" +
      "</div>";
    bodyHtml +=
      '<div style="background:#0d1117;border:1px solid rgba(34,197,94,.2);border-radius:3px;padding:14px 16px;margin-bottom:12px;border-left:3px solid #22c55e;">' +
      ...
      "</div>";
    bodyHtml +=
      '<div style="background:#111827;border:1px solid rgba(201,168,76,.15);border-radius:3px;padding:13px 16px;">' +
      ...
      '<p style="font-family:Scheherazade New,serif;font-size:1.15rem;direction:rtl;text-align:right;color:rgba(232,201,106,.75);line-height:1.7;display:block;margin-bottom:5px;">' +
      "\u0627\u0644\u062c\u0631\u062d \u0627\u0644\u0645\u0641\u0633\u064e\u0651\u0631 \u0645\u0642\u062f\u0651\u064e\u0645 \u0639\u0644\u0649 \u0627\u0644\u062a\u0639\u062f\u064a\u0644" +
      "</p>" +
  ...
  tabBar.style.cssText =
    "display:flex;background:rgba(9,5,0,.97);border-bottom:1px solid rgba(140,100,30,.18);position:sticky;top:0;z-index:10;";
  tabs.forEach(function (t) {
    var active = firaqTab === t[0];
    var btn = document.createElement("button");
    btn.style.cssText =
...
document.addEventListener("DOMContentLoaded", function () {
  loadHadithDatabase(
    ...
    function (err) {
      console.warn("[Al Mizan] DB erreur:", err);
    },
  );
  renderExamples();
  ...
  document.getElementById("hadith-input").addEventListener("keydown", function (e) {
    if (e.key === "Enter" && !e.shiftKey) {
      e.preventDefault();
      analyzeHadith();
    }
  });
  document.getElementById("home-input").addEventListener("keydown", function (e) {
    if (e.key === "Enter" && !e.shiftKey) {
      e.preventDefault();
      startHadithFromHome();
    }
...
console.log(
  "%c \u2705 M\u00eezan v21.0 : Pr\u00eat pour Production",
  "color: #00ff00; font-weight: bold;",
);
console.log(
  "%c \ud83d\udee1\ufe0f TRIPLE BOUCLIER ACTIF \u2014 Fallback Arbre Royal connect\u00e9",
  "color:#d4af37;font-weight:bold;",
);
