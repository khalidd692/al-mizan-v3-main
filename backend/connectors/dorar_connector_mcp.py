...
class DorarConnectorMCP:
    ...
    async def _simulate_dorar_response(self, hadith_id: int, book_key: str) -> Dict:
        ...
        return {
            ...
            "takhrij": f"رواه {book_name.split()[0]} في كتاب الإيمان، باب {hadith_id}",
    ...
    def _parse_dorar_hadith(self, raw_data: Dict, book_key: str) -> Dict:
        """
        Parse les données brutes vers le format Al-Mīzān
        """
        return {
    ...
    async def fetch_book_hadiths(self, book_key: str, start: int = 1, count: int = 100, 
        ...
        print(f"\n📖 Extraction {book_name}")
        print(f"   Range: {start} → {start + count - 1}")
        print(f"   Mode: {'MCP (réel)' if self.use_mcp else 'Simulation'}")
